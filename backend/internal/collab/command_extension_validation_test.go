...
func TestMixerPluginAndClipCommandsStayInValidationLockstep(t *testing.T) {
	...
	tests := []struct {
		...
		{"clip.setSampleEdit", map[string]any{"trackId": trackID.String(), "clipId": clipID.String(), "sampleEdit": extensionSampleEdit()}, []string{"clip:" + clipID.String() + ":sampleEdit", "project:tempoCascade", "track:" + trackID.String() + ":clipLanding"}},
