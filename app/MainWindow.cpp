...
/// The controller owns a project directory, while Finder or a file-open event
/// may hand us its inner manifest. Legacy `.vlt` directory packages and modern
/// ordinary project folders are both normalized to their containing directory.
QString packagePathFromSelection(const QString& path) {
    if (path.isEmpty()) return {};
    const QFileInfo info(path);
    const QString name = info.fileName();
    if (info.isFile() &&
        (info.suffix().compare(QStringLiteral("vlt"),
...
public:
    ...
    AudioAnalysisResultDialog(
        ...
        if (request.detectTempo &&
            ...
            m_tempo->setObjectName(QStringLiteral("DetectedTempoOptions"));
            m_tempo->setAccessibleName(tr("Detected BPM"));
            const auto tempoOptions =
                daw::analysis::applicableTempos(result.tempo);
            for (int bpm : tempoOptions)
                m_tempo->addItem(tr("%1 BPM").arg(bpm), bpm);
            column->addWidget(m_tempo);
            if (tempoOptions.size() > 1) {
                auto* showAll = new QToolButton(this);
                showAll->setObjectName(QStringLiteral("ShowAllTempoOptions"));
                showAll->setText(
                    tr("Show all BPM options (%1)").arg(tempoOptions.size()));
                showAll->setArrowType(Qt::DownArrow);
                showAll->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
                showAll->setAutoRaise(true);
                showAll->setCursor(Qt::PointingHandCursor);
                showAll->setAccessibleName(tr("Show all detected BPM options"));
                connect(showAll, &QToolButton::clicked, m_tempo,
                        [combo = m_tempo] { combo->showPopup(); });
                column->addWidget(showAll, 0, Qt::AlignLeft);
            }
            if (m_quickImport) {
...
bool checkAudioAnalysisResultDialogForTest(QWidget* parent) {
    daw::analysis::MusicalAnalysisResult result;
    result.tempo.status = daw::analysis::DetectionStatus::Ambiguous;
    result.tempo.bpm = 140.0;
    result.tempo.alternatives = {70.0, 280.0, 35.0};
    daw::analysis::MusicalAnalysisRequest request;
    request.detectTempo = true;
    request.detectKey = false;
    AudioAnalysisResultDialog dialog(
        result, request, 120.0, AudioAnalysisResultDialog::Mode::Review,
        false, parent);
    const auto* options =
...
MainWindow::MainWindow(bool openDevice, QWidget* parent,
    ...
    m_playheadTimer = new ui::FrameTimer(this);
    connect(m_playheadTimer, &ui::FrameTimer::timeout, this,
    ...
    setNotebookVisible(ui::notebookprefs::visible());
    ...
    if (!openDevice) (void)startRecovery(/*interactive=*/false);

    updateWindowTitle();
}

bool MainWindow::completeInteractiveStartup() {
    return startRecovery(/*interactive=*/true);
}
...
void MainWindow::setCloudSharedAssetMutationBridge(
    ...
    connect(bridge, &collab::CloudSharedAssetMutationBridge::cancelled, this,
            [this] {
                showTransientStatus(tr("Shared asset upload cancelled."),
                                         5000);
            });
}
...
void MainWindow::setCloudPublicationServices(
    ...
    collab::CloudAssetTransferManager* assetTransfers,
    collab::CloudRecordingAssetCoordinator* recordingAssets) {
    if (m_cloudPublisher)
    ...
    if (m_cloudRecordingAssets)
        disconnect(m_cloudRecordingAssets, nullptr, this, nullptr);
    ...
    if (m_sessionStrip) {
        m_sessionStrip->bindService(m_collaboration);
        m_sessionStrip->bindLifecycle(m_cloudSessionLifecycle);
        m_sessionStrip->bindSync(m_cloudProjectSync);
        m_sessionStrip->bindHydrator(m_cloudAssetHydrator);
    ...
    if (m_cloudPublisher) {
        connect(m_cloudPublisher,
                ...
                [this](quint64 generation,
                    ...
                    if (!message.isEmpty()) showTransientStatus(message);
                });
        connect(m_cloudPublisher,
                ...
                [this](quint64 generation,
                    ...
                    if (projectId.isEmpty() || !sameDocument) {
                        ...
                        QMessageBox::information(
                            ...
                            tr("The cloud copy was created, but this local project "
                               "changed while files were uploading. VLTONE kept the "
                    ...
                    if (!openCloudProject(projectId, sourcePath)) {
                        QMessageBox::warning(
                            ...
                            tr("The project was published, but VLTONE could not "
    ...
    if (m_cloudProjectSync) {
        ...
        connect(m_cloudProjectSync,
                ...
                this, [this](const QString& projectId, quint64,
                    ...
                    updateCloudSessionActions();
                    showTransientStatus(tr("Cloud project is ready"),
                                             5000);
                });
        connect(m_cloudProjectSync,
    ...
    if (m_cloudRecordingAssets) {
        connect(m_cloudRecordingAssets,
                &collab::CloudRecordingAssetCoordinator::progressChanged,
                this, [this](quint64 generation, qsizetype complete,
                             qsizetype total) {
                    if (generation != m_cloudRecordingAssetGeneration ||
                        total <= 0) {
                        return;
                    }
                    showTransientStatus(
                        tr("Uploading recorded audio… %1/%2")
                            .arg(complete)
                            .arg(total));
                });
        connect(m_cloudRecordingAssets,
                &collab::CloudRecordingAssetCoordinator::batchReady, this,
                &MainWindow::handleCloudRecordingAssetsReady);
        connect(m_cloudRecordingAssets,
                &collab::CloudRecordingAssetCoordinator::batchFailed, this,
                [this](quint64 generation, const QString& message,
                       bool retryable) {
                    if (generation != m_cloudRecordingAssetGeneration) return;
                    showTransientStatus(
                        message.isEmpty()
                            ? tr("Recorded audio is safely queued for upload")
                            : message.left(320),
                        8000, true);
                    if (!retryable) {
                        m_cloudRecordingAssetGeneration = 0;
                        m_cloudRecordingUploadRunId.clear();
                        m_cloudRecordingUploadOperationId.clear();
                    }
                });
        connect(m_cloudRecordingAssets, &QObject::destroyed, this, [this] {
            m_cloudRecordingAssets = nullptr;
            m_cloudRecordingAssetGeneration = 0;
        });
    }
    ...
    if (m_cloudSessionLifecycle) {
        ...
        connect(m_cloudSessionLifecycle,
                ...
                [this](const QString& safeMessage, bool error) {
                    ...
                    showTransientStatus(message, error ? 7000 : 5000);
                    if (error)
    ...
    if (m_collaborationCommandBridge) {
        ...
        connect(m_collaborationCommandBridge,
                &collab::CollaborationCommandBridge::operationDurablyObserved,
                this, [this](const QString& operationId, quint64, bool) {
                    cleanupCommittedCloudRecording(operationId);
                });
        connect(m_collaborationCommandBridge,
                &collab::CollaborationCommandBridge::operationDurabilityFailed,
                this, [this](const QString& operationId, const QString&,
                             const QString& message) {
                    if (operationId != m_cloudRecordingUploadOperationId)
                        return;
                    m_cloudRecordingAssetGeneration = 0;
                    m_cloudRecordingUploadRunId.clear();
                    m_cloudRecordingUploadOperationId.clear();
                    showTransientStatus(
                        message.isEmpty()
                            ? tr("Recorded audio remains queued for retry")
                            : message.left(320),
                        8000, true);
                });
    }
    ...
    if (m_recordingLeases) {
        ...
        connect(m_recordingLeases,
                ...
                [this](const collab::CloudClientError&,
                    ...
                    if (!m_cloudRecording ||
                        m_cloudRecording->leaseFreeNewClips) {
                        return;
                    }
...
void MainWindow::refreshPendingCloudRecordingPreviews() {
    if (!m_timeline) return;
    QVector<PendingCloudRecordingSpan> spans;
    if (!m_journal.running() || m_journal.sessionDir().empty() ||
        m_cloudProjectId.isEmpty()) {
        m_timeline->setPendingCloudRecordingSpans(std::move(spans));
        return;
    }

    daw::recovery::CloudRecordingRecoveryStore store(m_journal.sessionDir());
    daw::recovery::CloudRecordingRecoveryManifest manifest;
    if (!store.exists() || !store.read(manifest) ||
        manifest.projectId != m_cloudProjectId.toStdString()) {
        m_timeline->setPendingCloudRecordingSpans(std::move(spans));
        return;
    }
    for (const auto& run : manifest.runs) {
        for (const auto& capture : run.captures) {
            ...
            if (capture.passes.empty() && capture.durationSeconds > 0.0) {
                spans.push_back(
                    {QString::fromStdString(capture.trackId),
                     capture.startSeconds,
                     capture.startSeconds + capture.durationSeconds});
                continue;
            }
            for (const auto& pass : capture.passes) {
                if (pass.endSeconds <= pass.startSeconds) continue;
                spans.push_back({QString::fromStdString(capture.trackId),
...
void MainWindow::tryUploadPendingCloudRecording() {
    refreshPendingCloudRecordingPreviews();
    if (!m_cloudRecordingAssets || !m_collaborationCommandBridge ||
        !m_collaboration || !m_journal.running() ||
        m_journal.sessionDir().empty() ||
        m_collaboration->commandSchemaVersion() !=
            int(daw::collab::kProjectCommandSchemaVersion) ||
        !m_collaboration->canSubmitOperations()) {
        return;
    }
    if (m_cloudRecordingAssetGeneration != 0) {
        if (m_cloudRecordingAssets->state() ==
            collab::CloudRecordingAssetState::Failed) {
            m_cloudRecordingAssets->retryFailed();
        }
        return;
    }
    ...
    daw::recovery::CloudRecordingRecoveryManifest manifest;
    if (!store.read(manifest) ||
        manifest.projectId != m_cloudProjectId.toStdString() ||
        !m_cloudSessionLifecycle ||
        manifest.sessionId !=
            m_cloudSessionLifecycle->sessionId().toStdString()) {
        return;
    }

    const auto eligible = std::find_if(
        manifest.runs.cbegin(), manifest.runs.cend(), [](const auto& run) {
            return !run.recoveryOnly && !run.lostLease &&
                   std::all_of(run.captures.cbegin(), run.captures.cend(),
                               [](const auto& capture) {
                                   return capture.leaseId.empty() &&
                                          capture.status ==
                                              daw::recovery::CloudRecordingCaptureStatus::Ready;
                               });
        });
    if (eligible == manifest.runs.cend()) return;

    const QString runId = QString::fromStdString(eligible->runId);
    const collab::CloudRecordingRecoveryUploadResult prepared =
        collab::prepareCloudRecordingRecoveryUpload(manifest, runId);
    if (!prepared) return;

    quint64 expected = m_cloudRecordingAssets->generation() + 1;
    if (expected == 0) expected = 1;
    m_cloudRecordingAssetGeneration = expected;
    m_cloudRecordingUploadRunId = runId;
    m_cloudRecordingUploadOperationId = prepared.opId;
    if (m_cloudRecordingAssets->begin(prepared.recordings) != expected) {
        m_cloudRecordingAssetGeneration = 0;
        m_cloudRecordingUploadRunId.clear();
        m_cloudRecordingUploadOperationId.clear();
        return;
    }
    showTransientStatus(tr("Uploading recorded audio…"));
}

void MainWindow::handleCloudRecordingAssetsReady(
    quint64 generation,
    const QVector<collab::CloudRecordingAssetResult>& results) {
    if (generation != m_cloudRecordingAssetGeneration) {
        return;
    }
    if (results.isEmpty() || !m_collaborationCommandBridge ||
        !m_collaboration || !m_collaboration->canSubmitOperations()) {
        m_cloudRecordingAssetGeneration = 0;
        m_cloudRecordingUploadRunId.clear();
        m_cloudRecordingUploadOperationId.clear();
        return;
    ...
    daw::recovery::CloudRecordingRecoveryStore store(m_journal.sessionDir());
    daw::recovery::CloudRecordingRecoveryManifest manifest;
    if (!store.read(manifest)) return;
    const auto run = std::find_if(
        manifest.runs.cbegin(), manifest.runs.cend(), [this](const auto& item) {
            return item.runId == m_cloudRecordingUploadRunId.toStdString();
        });
    if (run == manifest.runs.cend() ||
        run->captures.size() != std::size_t(results.size())) {
        return;
    }
    ...
    const auto snapshot =
        m_collaborationCommandBridge->confirmedSnapshotAt(base);
    if (!snapshot) return;

    QHash<QString, daw::AssetRef> assetsByUpload;
    for (const auto& result : results) {
        if (result.projectId != m_cloudProjectId ||
            assetsByUpload.contains(result.uploadId)) {
            return;
        }
        assetsByUpload.insert(result.uploadId, result.asset);
    }
    ...
    input.meta.baseServerSequence = base;
    const auto stableId = [](std::string_view domain,
                             const std::string& captureId,
                             std::size_t index = 0) {
        return daw::collab::deterministicMigrationId(
            domain, captureId + ":" + std::to_string(index));
    };
    for (const auto& capture : run->captures) {
        const auto asset = assetsByUpload.constFind(
            QString::fromStdString(capture.uploadId));
        if (asset == assetsByUpload.cend()) return;
        daw::collab::RecordingCommitCaptureInput planned;
        planned.captureId = capture.captureId;
        planned.uploadedAsset = asset.value();
        if (capture.semantics.mode ==
            daw::recovery::CloudRecordingMode::Overwrite) {
            std::string afterId;
            for (std::size_t index = 0; index < capture.passes.size(); ++index) {
                const std::string clipId =
                    stableId("recording-clip", capture.captureId, index);
                planned.clips.push_back({clipId, afterId});
                afterId = clipId;
            }
        } else {
            planned.containerClipId =
                stableId("recording-container", capture.captureId);
            std::string afterId;
            for (std::size_t index = 0; index < capture.passes.size(); ++index) {
                const std::string takeId =
                    stableId("recording-take", capture.captureId, index);
                planned.takes.push_back({takeId, afterId});
                afterId = takeId;
            }
            const auto count =
                daw::collab::RecordingCommitPlanner::requiredCompSegmentCount(
                    capture);
            if (!count) return;
            afterId.clear();
            for (std::size_t index = 0; index < *count; ++index) {
                const std::string segmentId =
                    stableId("recording-comp", capture.captureId, index);
                planned.compSegments.push_back({segmentId, afterId});
                afterId = segmentId;
            }
        }
    ...
    if (!plan || !plan.command) {
        showTransientStatus(
            tr("Recorded audio is uploaded but its project edit must be retried"),
            8000, true);
        m_cloudRecordingAssetGeneration = 0;
        return;
    }
...
void MainWindow::cleanupCommittedCloudRecording(
    ...
    if (!m_journal.running() || m_journal.sessionDir().empty() ||
        canonicalCloudUuid(operationId) != operationId) {
        return;
    }
    daw::recovery::CloudRecordingRecoveryStore store(m_journal.sessionDir());
    if (!store.exists()) return;
    ...
    if (!store.read(manifest)) return;
    const auto run = std::find_if(
        manifest.runs.cbegin(), manifest.runs.cend(),
        [&operationId](const auto& item) {
            return item.opId == operationId.toStdString();
        });
    if (run == manifest.runs.cend()) return;
    const std::string runId = run->runId;
    if (!store.removeRunAfterCommit(runId)) return;
    refreshPendingCloudRecordingPreviews();
    if (operationId == m_cloudRecordingUploadOperationId) {
        m_cloudRecordingAssetGeneration = 0;
        m_cloudRecordingUploadRunId.clear();
    ...
    showTransientStatus(tr("Recording committed to the shared project"),
                        5000);
    QTimer::singleShot(0, this,
                       &MainWindow::tryUploadPendingCloudRecording);
}
...
void MainWindow::startCloudRecording(
    ...
    if (!ensureCloudRecordingRecoverySession()) {
        QMessageBox::warning(
            ...
            tr("VLTONE could not create durable local recovery storage. Recording was not started."));
        return;
    }
    ...
    if (recovery.exists()) {
        ...
        if (!read || existing.projectId != projectId.toStdString() ||
            ...
            QMessageBox::warning(
                ...
                tr("A previous cloud take still needs recovery. VLTONE will not replace it with a different project or session."));
            return;
        }
    }
...
void MainWindow::handleRecordingLeasesAcquired() {
    ...
    const bool leaseFree = m_cloudRecording->leaseFreeNewClips;
    if (!leaseFree && !m_recordingLeases) return;
    ...
    m_cloudRecording->recoveryOnly = false;
...
void MainWindow::handleCloudRecordingContextChange() {
    ...
    if (m_recordingLeases)
        m_recordingLeases->handleSessionChanged(currentProject,
                                                currentSession);
    tryUploadPendingCloudRecording();
}
...
void MainWindow::updateCloudSessionActions() {
    ...
    if (m_startSessionAction) {
        const bool activate = available &&
            m_cloudSessionLifecycle->canActivateSession();
        m_startSessionAction->setEnabled(
            activate || (available &&
                         m_cloudSessionLifecycle->canStartSession()));
        m_startSessionAction->setText(
            activate ? tr("Start Collaboration Session")
                     : tr("Create Collaboration LobbyвЂ¦"));
        m_startSessionAction->setToolTip(
            activate
                ? tr("Start after every participant is ready or remains a viewer.")
                : available
...
bool MainWindow::openCloudProject(const QString& requestedProjectId,
    ...
    showTransientStatus(tr("Opening verified cloud project…"));
    return true;
}
...
void MainWindow::onOpenCloudProjects() {
    ...
    collab::CloudProjectsDialog dialog(m_cloudProjectClient, m_cloudProjectId,
                                       this);
    // Publish and Join both replace this dialog rather than nesting a second
    // modal inside it: the old flow ran the join box inside this one's event
    // loop, which is what made a bootstrap completing underneath it able to
    // steal the dialog's result.
    QString joinedProject;
    connect(&dialog, &collab::CloudProjectsDialog::joinByCodeRequested, &dialog,
            [this, &dialog, &joinedProject] {
                joinedProject = openJoinSessionDialog({}, &dialog);
                if (!joinedProject.isEmpty()) dialog.reject();
            });
    connect(&dialog, &collab::CloudProjectsDialog::inviteRequested, &dialog,
            [this, &dialog](const QString& projectId) {
                if (projectId.isEmpty()) return;
                collab::CloudProjectInviteDialog invite(m_cloudProjectClient,
                                                        projectId, &dialog);
                invite.exec();
            });
    ...
    if (!joinedProject.isEmpty()) return;
    if (result != QDialog::Accepted) return;
    const QString chosen = dialog.chosenProjectId();
    if (!chosen.isEmpty()) openCloudProject(chosen);
#endif
}
...
void MainWindow::onMakeLocalCopy() {
    if (m_cloudProjectId.isEmpty() || !m_cloudProjectSync ||
        ...
        m_cloudProjectSync->phase() != collab::CloudSyncPhase::Ready) {
        showTransientStatus(tr("No verified cloud project is open"), 5000);
        return;
    }
    ...
    if (targetInfo.exists() && !targetInfo.isDir()) {
        QMessageBox::warning(
            ...
            tr("The selected destination is not a VLTONE project folder."));
        return;
    }
    ...
    const QString finalManifestName = QFileInfo(QString::fromStdString(
        daw::ProjectSerializer::manifestPath(target.toStdString()))).fileName();

    QApplication::setOverrideCursor(Qt::WaitCursor);
    audio::Result saved = m_controller.saveProject(staging.toStdString());
    if (saved) {
        const QString stagingManifest = QString::fromStdString(
            daw::ProjectSerializer::manifestPath(staging.toStdString()));
        ...
        if (saved) {
            ...
            const QString finalManifest =
                QDir(staging).absoluteFilePath(finalManifestName);
            saved = daw::ProjectSerializer::saveDocument(
                portable, finalManifest.toStdString(),
                daw::MediaPaths::Basenames);
            if (saved && stagingManifest != finalManifest &&
                QFileInfo::exists(stagingManifest) &&
                !QFile::remove(stagingManifest)) {
                saved = audio::Result::fail(
                    audio::EngineError::FileWriteError,
                    "could not remove the temporary project manifest");
            }
        }
    }
...
void MainWindow::showTransientStatus(const QString& safeMessage,
                                     int timeoutMs, bool error) {
#ifdef DAW_ENABLE_COLLABORATION
    if (m_sessionStrip && !m_sessionStrip->isHidden()) {
        m_sessionStrip->showNotice(safeMessage, error, timeoutMs);
        return;
    }
#endif
    // Local projects use the ordinary status bar without revealing the
    // collaboration strip just to show a notice.
    statusBar()->showMessage(safeMessage, timeoutMs);
}
...
void MainWindow::onStartCollaborationSession() {
#ifdef DAW_ENABLE_COLLABORATION
    if (!m_cloudSessionLifecycle) {
        showTransientStatus(
    ...
    if (m_cloudSessionLifecycle->canActivateSession()) {
        if (!m_cloudSessionLifecycle->activateSession())
            showTransientStatus(tr("The collaboration lobby could not be started."),
                                5000, true);
        return;
    }

    QDialog dialog(this);
    dialog.setWindowTitle(tr("Start Collaboration Session"));
    dialog.setWindowFlag(Qt::WindowContextHelpButtonHint, false);
    dialog.setModal(true);
    dialog.setMinimumWidth(collab::dialog::kMinimumWidth);

    QLabel* title =
        collab::dialog::titleLabel(tr("Start a session"), &dialog);
    auto* description = new QLabel(
        tr("People you invite join with the invitation code. A password is "
           "optional and protects the room itself, so a code that leaks is "
           "still not enough to walk in."),
        &dialog);
    ...
    hint->setObjectName(QStringLiteral("CollabWarning"));
    hint->setWordWrap(true);
    hint->hide();

    auto* start = new QPushButton(tr("Start session"), &dialog);
    ...
    dialog.setStyleSheet(collab::dialog::styleSheet());

    // The floor mirrors auth.ValidateSessionSecret on the server. Enforcing it
    // here means a doomed password is never put on the wire at all.
    const auto acceptable = [&] {
        return !protect->isChecked() || password->text().size() >= 6;
    };
    const auto syncState = [&] {
        password->setEnabled(protect->isChecked());
        start->setEnabled(acceptable());
        hint->setVisible(protect->isChecked() && !acceptable() &&
                         !password->text().isEmpty());
        hint->setText(tr("Use at least 6 characters."));
    };
    connect(protect, &QCheckBox::toggled, &dialog, [&](bool on) {
        syncState();
        if (on) password->setFocus();
    });
    connect(password, &QLineEdit::textChanged, &dialog,
            [&] { syncState(); });
    ...
    const int result = dialog.exec();
    QString secret = protect->isChecked() ? password->text() : QString();
    // Overwrite the widget's copy before anything else can read it back.
    password->clear();
...
void MainWindow::onSessionSettings() {
    ...
    participants->setColumnCount(4);
    participants->setHeaderLabels(
        {tr("Participant"), tr("Role"), tr("Session"), tr("Plugins")});
    participants->setAccessibleName(
        tr("Session participants and plugin readiness"));
    participants->setRootIsDecorated(true);
    ...
    participants->header()->setSectionResizeMode(2,
                                                  QHeaderView::ResizeToContents);
    participants->header()->setSectionResizeMode(3,
                                                  QHeaderView::ResizeToContents);
    ...
    auto* pluginActions = new QHBoxLayout;
    auto* rescanPlugins = new QPushButton(tr("Rescan Plugins"), &dialog);
    auto* checkPlugins = new QPushButton(tr("Check Again"), &dialog);
    auto* openPlugins = new QPushButton(tr("Open Plugin Manager"), &dialog);
    auto* stayViewer = new QPushButton(tr("Stay as Viewer"), &dialog);
    rescanPlugins->setAccessibleDescription(
        tr("Scans installed plugins in isolated helper processes."));
    checkPlugins->setAccessibleDescription(
        tr("Checks this computer against the session plugin manifest."));
    stayViewer->setAccessibleDescription(
        tr("Keeps this participant read-only without blocking the lobby."));
    pluginActions->addWidget(rescanPlugins);
    pluginActions->addWidget(checkPlugins);
    pluginActions->addWidget(openPlugins);
    pluginActions->addStretch(1);
    pluginActions->addWidget(stayViewer);
    layout->addLayout(pluginActions);
    ...
    bool pluginSignalConnected = false;
    ...
    const auto localSessionMember = [&]()
        -> const collab::CloudSessionMember* {
        const QString local = m_collaboration
            ? m_collaboration->localParticipantId()
            : QString();
        const QString user = client ? client->currentUserId() : QString();
        const collab::CloudSessionMember* sameUser = nullptr;
        for (const auto& member : session.members) {
            if (!local.isEmpty() && member.id == local) return &member;
            if (!user.isEmpty() && member.userId == user && !sameUser)
                sameUser = &member;
        }
        return sameUser;
    };
    ...
    const auto updateControls = [&] {
        ...
        checkPlugins->setEnabled(v3Room && !busy && !scanning);
        openPlugins->setEnabled(!busy);
        stayViewer->setEnabled(v3Room && !busy && localSessionMember());
    };
    ...
    const auto render = [&] {
        ...
        const auto addParticipant = [&](const QString& userId, int rowRole,
            ...
            const collab::CloudSessionMember* sessionMember = nullptr;

            if (sessionActive) {
                for (const collab::CloudSessionMember& member : session.members) {
                    if (member.userId != userId) continue;
                    if (!sessionMember || member.id == localParticipant)
            ...
            QString pluginState = tr("Not joined");
            QStringList pluginDetails;
            if (sessionMember) {
                if (sessionMember->readinessStatus == QLatin1String("ready"))
                    pluginState = tr("Ready");
                else if (sessionMember->readinessStatus ==
                         QLatin1String("viewer"))
                    pluginState = tr("Viewer");
                else
                    pluginState = tr("Needs attention");
                for (const auto& result : sessionMember->pluginReadiness) {
                    QString stateName;
                    switch (result.status) {
                        case daw::collab::PluginReadinessStatus::Ready:
                            stateName = tr("Ready");
                            break;
                        case daw::collab::PluginReadinessStatus::Missing:
                            stateName = tr("Missing");
                            break;
                        case daw::collab::PluginReadinessStatus::VersionMismatch:
                            stateName = tr("Version mismatch");
                            break;
                        case daw::collab::PluginReadinessStatus::ProbeFailed:
                            stateName = tr("Probe failed");
                            break;
                    }
                    pluginDetails.push_back(
                        QStringLiteral("%1 · %2 — %3")
                            .arg(QString::fromStdString(
                                     result.requirement.nativeUid),
            ...
            auto* item = new QTreeWidgetItem(
                participants, {name, roleName, sessionState, pluginState});
            ...
            const QString pluginTooltip = pluginDetails.isEmpty()
                ? pluginState
                : pluginDetails.join(QLatin1Char('\n'));
            item->setToolTip(3, pluginTooltip);
            for (const QString& detail : pluginDetails) {
                auto* pluginItem = new QTreeWidgetItem(
                    item, {detail, QString(), QString(), QString()});
                pluginItem->setFlags(pluginItem->flags() &
                                     ~Qt::ItemIsSelectable);
            }
            item->setExpanded(
                !pluginDetails.isEmpty() &&
                ((client && userId == client->currentUserId()) ||
                 pluginState == tr("Needs attention")));
        ...
        if (projectRequest || membersRequest || sessionRequest) {
            nextStatus = tr("Refreshing project participants\u2026");
        } else if (mutationRequest) {
            nextStatus = tr("Applying session settings\u2026");
        } else if (!loadError.isEmpty()) {
            nextStatus = loadError;
        } else if (sessionLoaded && sessionActive) {
            nextStatus =
                session.session.status == collab::CloudSessionStatus::Starting
                    ? tr("Lobby: check plugin readiness, then the host can start the session.")
                    : tr("Live session: %1 participant(s) joined.")
                          .arg(session.members.size());
        } else {
            nextStatus = tr("No live session is active. Project membership is still available below.");
        }
        if (projectLoaded && !currentOwner() && !currentHost()) {
            nextStatus +=
                tr("\nProject management is read-only for your role.");
        }
        if (status->text() != nextStatus) {
            status->setText(nextStatus);
            status->setAccessibleName(nextStatus);
            QAccessibleEvent event(status, QAccessible::NameChanged);
            ...
            [&](quint64 requestId, collab::CloudRequestKind kind,
                ...
                if (requestId == mutationRequest &&
                    (kind == collab::CloudRequestKind::HandoffSession ||
                     kind == collab::CloudRequestKind::UpdateSessionReadiness) &&
                    result.session.projectId == projectId) {
                    mutationRequest = 0;
                    session = result;
                    requestRefresh();
    ...
    connect(checkPlugins, &QPushButton::clicked, &dialog,
            [&] { submitPluginReadiness(false); });
    connect(stayViewer, &QPushButton::clicked, &dialog,
            [&] { submitPluginReadiness(true); });
    connect(openPlugins, &QPushButton::clicked, &dialog,
            [this] { openPluginManager(); });
    connect(rescanPlugins, &QPushButton::clicked, &dialog, [&] {
        openPluginManager();
        if (!pluginSignalConnected && m_pluginManagerWindow) {
            pluginSignalConnected = true;
            connect(m_pluginManagerWindow,
                    &PluginManagerWindow::pluginsChanged, &dialog,
                    [&] { submitPluginReadiness(false); });
        }
        m_controller.pluginManager().startScan(true);
        status->setText(
            tr("Scanning plugins in isolated helper processes\u2026"));
...
void MainWindow::onPublishCloudProject() {
    ...
    if (m_projectPath.isEmpty() || m_dirty ||
        ...
        QMessageBox::warning(
            ...
            tr("VLTONE could not confirm a clean local project folder. Save the "
               "project and try again."));
        return;
    }

    showTransientStatus(tr("Capturing built-in plugin state…"));
    QApplication::setOverrideCursor(Qt::WaitCursor);
    std::unique_ptr<daw::cloud::CloudPublicationCapture> capture;
    try {
        ...
        QMessageBox::warning(
            ...
            tr("VLTONE could not stage a safe cloud copy. The local project was "
               "not changed."));
        return;
    }
    QApplication::restoreOverrideCursor();
    ...
    if (!capture->readyForHashing()) {
        ...
        showTransientStatus(tr("Project is not ready for cloud publication"),
                                 6000);
        QMessageBox::warning(
    ...
    showTransientStatus(tr("Checking project for cloud publication…"));
}
...
bool MainWindow::startRecovery(bool interactive) {
    bool restoredAtStartup = false;
    ...
    if (!interactive && !std::getenv("DAW_RECOVERY_ROOT")) return false;
    ...
    if (!ui::recoveryprefs::enabled()) return false;
    ...
    m_journal.start(root.toStdString(),
                    QCoreApplication::applicationVersion().toStdString());
    if (!m_journal.running()) return restoredAtStartup;
...
bool MainWindow::checkAutomationForTest() {
    ...
    // ── Shift-click adds a timing anchor at the curve's current value ──
    {
        const std::vector<daw::AutomationPoint> pointsBefore = curve()->points;
        const double fallback = curve()->defaultValue;
        const QPoint anchorAt(m_timeline->width() * 2 / 5, y + 16);
        click(anchorAt, anchorAt, Qt::ShiftModifier);
        const daw::ClipAutomationModel* anchored = curve();
        const daw::AutomationPoint* added = nullptr;
        if (anchored) {
            for (const auto& candidate : anchored->points) {
                const bool existed = std::any_of(
                    pointsBefore.begin(), pointsBefore.end(),
                    [&](const daw::AutomationPoint& point) {
                        return std::abs(point.beats - candidate.beats) < 1e-9;
                    });
                if (!existed) added = &candidate;
            }
        }
        if (!anchored || anchored->points.size() != pointsBefore.size() + 1 ||
            !added ||
            std::abs(added->value - daw::automationValueAt(
                                        pointsBefore, added->beats, fallback)) >
                1e-9) {
            std::fprintf(stderr,
                         "Shift-click did not add a point on the existing curve\n");
            return false;
        }
        m_controller.undo();
        QApplication::processEvents();
        if (!curve() || curve()->points != pointsBefore) {
            std::fprintf(stderr,
                         "undo did not remove the Shift-click automation point\n");
            return false;
        }
    }
...
bool MainWindow::checkAutomationEditorForTest() {
    ...
    // A short Shift-click and a Shift range drag share the same initial press.
    // The click adds an exact timing anchor; movement beyond the hysteresis
    // remains the range-selection gesture tested by the screenshot harness.
    {
        const std::vector<daw::AutomationPoint> pointsBefore = curve()->points;
        const double fallback = curve()->defaultValue;
        const QPoint first = view->pointPositionForTest(0);
        const QPoint second = view->pointPositionForTest(1);
        const QPoint anchorAt((first.x() + second.x()) / 2,
                              view->height() - 40);
        sendView(QEvent::MouseButtonPress, anchorAt, Qt::LeftButton,
                 Qt::LeftButton, Qt::ShiftModifier);
        sendView(QEvent::MouseButtonRelease, anchorAt, Qt::LeftButton,
        ...
        if (!anchored || anchored->points.size() != pointsBefore.size() + 1 ||
            ...
            std::abs(added->value - daw::automationValueAt(
                ...
                1e-9) {
            std::fprintf(stderr,
                         "the editor's Shift-click missed the existing curve\n");
            editor->close();
            return false;
        }
        m_controller.undo();
        ...
        if (!curve() || curve()->points != pointsBefore) {
            std::fprintf(stderr,
                         "undo did not remove the editor's Shift-click point\n");
            editor->close();
            return false;
        }
    }
...
bool MainWindow::checkKnobAutomationForTest() {
    ...
    if (createMode->isChecked() || ui::automationCreationMode() ||
        m_controller.automationLanesOf(track).size() != lanesBefore) {
        std::fprintf(stderr, "Alt/Option still enables automation creation\n");
        editor->close();
        return false;
    }
    ...
    const auto createFromMenu = [&] {
        ...
        QApplication::sendEvent(knob, &menu);
        QApplication::processEvents();
    };
    createFromMenu();
    ...
    if (lanes.size() != lanesBefore + 1) {
        std::fprintf(stderr,
                     "the knob context menu made %d lanes, wanted one more than %d\n",
    ...
    if (m_controller.automationLanesOf(track).size() != lanes.size()) {
        std::fprintf(stderr,
                     "creating automation for the same knob twice made two lanes\n");
        editor->close();
        return false;
    }
    ...
    QApplication::sendEvent(knob, &altRelease);
    if (!createMode->isChecked() || !ui::automationCreationMode()) {
        std::fprintf(stderr, "Alt/Option changed the explicitly enabled automation mode\n");
        editor->close();
        return false;
    }
    doubleClick(Qt::NoModifier);
    ...
    // The same rule covers every shared parameter control, including the
    // compact context-strip slider and track/mixer volume and pan.
    ui::FaderWidget fader;
    ui::PanKnob pan;
    ui::MiniSlider mini(icons::Glyph::Volume, QString());
    int requests = 0;
    fader.setAutomatable(true);
    pan.setAutomatable(true);
    mini.setAutomatable(true);
    connect(&fader, &ui::FaderWidget::automateRequested, this, [&] { ++requests; });
    connect(&pan, &ui::PanKnob::automateRequested, this, [&] { ++requests; });
    connect(&mini, &ui::MiniSlider::automateRequested, this, [&] { ++requests; });
    const auto doubleClickControls = [&](Qt::KeyboardModifiers mods) {
        for (QWidget* control : {static_cast<QWidget*>(&fader),
                                static_cast<QWidget*>(&pan),
                                static_cast<QWidget*>(&mini)}) {
            const QPoint at = control->rect().center();
            QMouseEvent twice(QEvent::MouseButtonDblClick, QPointF(at),
                              QPointF(control->mapToGlobal(at)), Qt::LeftButton,
                              Qt::LeftButton, mods);
            QApplication::sendEvent(control, &twice);
        }
    };
    doubleClickControls(Qt::AltModifier);
    if (requests != 0) return false;
    createMode->click();
    doubleClickControls(Qt::NoModifier);
    createMode->click();
    if (requests != 3) return false;

    const QString toggleId = QStringLiteral("tool.automationCreationToggle");
    const QKeySequence toggleKey(Qt::CTRL | Qt::SHIFT | Qt::Key_U);
    if (m_shortcuts->defaultShortcut(toggleId) != toggleKey ||
        !m_shortcuts->conflict(toggleId, toggleKey).isEmpty()) {
        std::fprintf(stderr, "automation toggle has a missing or conflicting default shortcut\n");
        return false;
    }
    // Russian Г is the physical U key. This also exercises the real shortcut
    // dispatcher instead of invoking the toolbar command directly.
    const auto pressU = [&](Qt::KeyboardModifiers modifiers) {
        QKeyEvent key(QEvent::KeyPress, 0x01000413, modifiers,
                      QString(QChar(0x0413)));
        QApplication::sendEvent(knob, &key);
    };
    knob->setFocus();
    pressU(Qt::ControlModifier | Qt::ShiftModifier);
    if (!createMode->isChecked() || !ui::automationCreationMode()) return false;
    pressU(Qt::ControlModifier | Qt::ShiftModifier);
    if (createMode->isChecked() || ui::automationCreationMode()) return false;

    QAction altShortcut(this);
    altShortcut.setShortcut(QKeySequence(Qt::ALT | Qt::Key_U));
    altShortcut.setShortcutContext(Qt::ApplicationShortcut);
    int altTriggered = 0;
    connect(&altShortcut, &QAction::triggered, this, [&] { ++altTriggered; });
    QApplication::sendEvent(knob, &altPress);
    pressU(Qt::AltModifier);
    QApplication::sendEvent(knob, &altRelease);
    if (altTriggered != 1 || createMode->isChecked() || ui::automationCreationMode()) {
        std::fprintf(stderr, "Alt/Option shortcut no longer dispatches independently\n");
        return false;
    }
    std::fprintf(stderr, "PASS Automation creation: context menu, explicit mode, toggle shortcut and independent Alt/Option\n");

    editor->close();
    // The context menu and shortcut checks explicitly moved keyboard focus
    // into the editor. Return it to the workspace for subsequent input tests.
    activateWindow();
...
bool MainWindow::checkCycleRegionForTest() {
    ...
    // This assertion starts a fresh run. In Restart mode a seek while playing
    // deliberately preserves the old play anchor, which another UI check may
    // have left inside this range.
    m_controller.pause();
    m_controller.seekSeconds(0.0);
    m_controller.play();
    const double startedAt = m_controller.positionSeconds();
    const bool jumpedIn = std::abs(startedAt - from) < 1e-6;
    m_controller.stop();
    if (!jumpedIn) {
        std::fprintf(stderr, "playing from outside the cycle did not enter it: expected=%.9f actual=%.9f range=%.9f..%.9f enabled=%d rate=%.1f\n",
                     from, startedAt, m_controller.loopStartSeconds(), m_controller.loopEndSeconds(),
                     m_controller.isLoopEnabled(), m_controller.sampleRate());
        return false;
    }
    ...
    const auto awaitCursor = [this](double seconds) {
        const double expected = (seconds - m_timeline->horizontalScrollForTest()) *
                                m_timeline->pixelsPerSecondForTest();
        QElapsedTimer wait; wait.start();
        while (std::abs(m_timeline->displayedPlayheadSubpixelXForTest() - expected) > 1e-8 &&
               wait.elapsed() < 400) {
            QApplication::processEvents();
            QThread::msleep(1);
        }
        return std::abs(m_timeline->displayedPlayheadSubpixelXForTest() - expected) <= 1e-8;
    };
    m_controller.seekSeconds(0.25);
    m_timeline->update();
    if (!awaitCursor(0.25)) return false;
    ...
    if (!awaitCursor(0.50)) return false;
...
bool MainWindow::checkTrackSelectionForTest() {
    ...
    // ratio, so the balance they started with survives.
    {
        ...
        const QPoint to = fader->isCompactKnob()
            ? from + QPoint(0, 24)
            : QPoint(fader->width() / 2, fader->height() / 2);
        const auto send = [&](QEvent::Type type, const QPoint& at,
                              Qt::MouseButton button, Qt::MouseButtons held) {
    ...
    // pair, so neither channel control disappears with the full fader throw.
    {
        ...
        // Virtual rows own controls only near the viewport. Exercise the deep
        // row after scrolling it into view, as an actual pointer gesture does.
        const auto& visible = daw::visibleTracks(m_controller.project());
        for (std::size_t lane = 0; lane < visible.size(); ++lane) {
            if (m_controller.project().tracks[visible[lane].index].id == leaf) {
                m_timeline->ensureLaneVisible(int(lane));
                break;
            }
        }
        QApplication::processEvents();
        ...
        if (!deepControls.second) {
            auto* fader = m_trackList->rowFaderForTest(deep);
            std::fprintf(stderr, "a deeply nested row lost its compact pan: width=%d rowY=%d scroll=%d fader=%d rowH=%d\n",
                         m_trackList->width(), m_trackList->rowRectForTrack(deep).y(), m_timeline->verticalScroll(),
                         fader != nullptr, m_trackList->rowRectForTrack(deep).height());
            return false;
        }
...
bool MainWindow::checkTimelinePanForTest() {
    ...
    // The pointer itself chooses the anchor. Inside the arrangement, the time
    // beneath it stays fixed. Above it, toolbar/keyboard zoom centres the
    // playhead, irrespective of clip selection.
...
bool MainWindow::checkTimelineClipGesturesForTest() {
    ...
    const QPointF scaleTo = scaleFrom - QPointF(0.0, 20.0);
    ...
    const auto originalHeights = [&] {
        ...
        for (const auto& track : m_controller.project().tracks)
            values.emplace_back(track.id, track.height);
        return values;
    }();
    const std::size_t heightUndoMark = m_controller.undoDepth();
    const int heightBaseline = trackHeightSlider->value();
    dragCompactControl(trackHeightSlider, QPointF(0.0, -6.0), 2);
    const int heightTarget = trackHeightSlider->value();
    const bool upwardDragExpanded =
        heightBaseline == trackHeightSlider->maximum()
            ? heightTarget == heightBaseline
            : heightTarget > heightBaseline;
    const bool heightThrowsAccumulated =
        heightBaseline > trackHeightSlider->maximum() - 16 ||
        heightTarget >= heightBaseline + 16;
    bool allHeightsChanged = !m_controller.project().tracks.empty();
    for (const auto& [trackId, storedHeight] : originalHeights) {
        const auto* track = m_controller.project().findTrack(trackId);
        const int expected = std::clamp(
            int(std::lround(ui::laneHeightFor(storedHeight) *
                            double(heightTarget) / std::max(1, heightBaseline))),
            ui::kMinLaneHeight, ui::kMaxLaneHeight);
        allHeightsChanged = allHeightsChanged && track &&
                            ui::laneHeightFor(track->height) == expected;
    }
    const bool oneHeightUndo =
        m_controller.undoDepth() ==
        std::min(heightUndoMark + 1, m_controller.undoLimit());
    if (oneHeightUndo) m_controller.undo();
    for (const auto& [trackId, height] : originalHeights)
        m_controller.setTrackHeight(trackId, height);
    if (!originalHeights.empty())
        m_toolPanel->setTrackHeightValue(
            ui::laneHeightFor(originalHeights.front().second));
    ...
    const double positionBeforeSlider = m_controller.positionSeconds();
    m_controller.seekSeconds(centreBeforeSlider);
    QCursor::setPos(timelineZoomSlider->mapToGlobal(
        timelineZoomSlider->rect().center()));
    const int zoomBaseline = timelineZoomSlider->value();
    dragCompactControl(timelineZoomSlider, QPointF(5.0, 0.0), 2);
    const bool rightDragZoomedIn =
        zoomBaseline == timelineZoomSlider->maximum()
            ? timelineZoomSlider->value() == zoomBaseline
            : timelineZoomSlider->value() > zoomBaseline;
    const bool zoomThrowsAccumulated =
        zoomBaseline > timelineZoomSlider->maximum() - 9 ||
        timelineZoomSlider->value() >= zoomBaseline + 9;
    const double centreAfterSlider =
        m_timeline->horizontalScrollForTest() +
        anchorX / m_timeline->pixelsPerSecondForTest();
    const bool zoomCentredOnPlayhead =
        std::abs(m_timeline->pixelsPerSecondForTest() - scaleBeforeSlider) > 0.1 &&
        std::abs(centreAfterSlider - centreBeforeSlider) < 1.0e-6;
    m_timeline->setHorizontalZoom(scaleBeforeSlider);
    QCursor::setPos(cursorBeforeSlider);
    m_controller.seekSeconds(positionBeforeSlider);
    if (!compactControls || !upwardDragExpanded || !heightThrowsAccumulated ||
        !allHeightsChanged || !oneHeightUndo || !rightDragZoomedIn ||
        !zoomThrowsAccumulated || !zoomCentredOnPlayhead) {
        std::fprintf(stderr,
                     "compact timeline controls failed (size=%d up=%d "
                     "height-repeat=%d height=%d undo=%d right=%d "
                     "zoom-repeat=%d centre=%d)\n",
                     int(compactControls), int(upwardDragExpanded),
                     int(heightThrowsAccumulated), int(allHeightsChanged),
                     int(oneHeightUndo), int(rightDragZoomedIn),
    ...
    // Exercise the actual editor child: that is where mouse/keyboard events
    // land, and a wheel must continue through it to the inspector's scroller.
    m_inspector->syncFromModel();
    auto* pitchEdit = pitch->findChild<QLineEdit*>();
    auto* clipScroll = m_inspector->findChild<QScrollArea*>("InspectorScrollArea");
    auto* clipSection = m_inspector->findChild<QWidget*>("InspectorClipSection");
    auto* moreClip = m_inspector->findChild<QToolButton*>("InspectorMoreClipSettings");
    if (!pitchEdit || !clipScroll || !clipSection || !moreClip) return false;
    const auto inspectorCheck = [](bool passed, const char* detail) {
        std::fprintf(stderr, "%s Inspector %s\n", passed ? "PASS" : "FAIL", detail);
        return passed;
    };
    const auto clipWheel = [](QWidget* target, QPoint pixels = {}) {
        const QPointF at(target->rect().center());
        QWheelEvent event(at, target->mapToGlobal(at.toPoint()), pixels,
                          pixels.isNull() ? QPoint(0, -120) : QPoint(),
    ...
    const auto pitchKey = [&](int key, const QString& text = {}) {
        ...
        QApplication::sendEvent(pitchEdit, &shortcut);
        QKeyEvent event(QEvent::KeyPress, key, Qt::NoModifier, text);
        QApplication::sendEvent(pitchEdit, &event);
    };
    const auto pitchValue = [&] {
        return m_controller.clipSampleParameter(audioTrack, audioClip, "stretch.pitch");
    };
    ...
    if (!inspectorCheck(fitsInspector(), "basic settings fit with side margins")) return false;
    moreClip->setChecked(true);
    if (!inspectorCheck(fitsInspector(), "expanded settings fit with side margins")) return false;
    // Long, unbroken sample filenames must not widen the entire content pane.
    auto* clipName = m_inspector->findChild<QLabel*>("InspectorClipName");
    const QString nameBefore = clipName->text();
    clipName->setText(QString(160, QLatin1Char('W')));
    if (!inspectorCheck(fitsInspector(), "long sample names stay inside the panel")) return false;
    clipName->setText(nameBefore);
    for (auto* field : clipSection->findChildren<QDoubleSpinBox*>()) {
    ...
    clipWheel(pitchEdit);
    if (!inspectorCheck(clipScroll->verticalScrollBar()->value() > 0,
                        "wheel over a number scrolls the panel")) return false;
    clipScroll->verticalScrollBar()->setValue(0);
    ...
    pitchMouse(QEvent::MouseButtonDblClick, 0);
    pitchKey(Qt::Key_4, QStringLiteral("4"));
    pitchKey(Qt::Key_unknown, pitch->locale().decimalPoint());
    pitchKey(Qt::Key_5, QStringLiteral("5"));
    const QString pendingText = pitchEdit->text();
    m_inspector->syncFromModel();
    clipWheel(pitchEdit);
    clipWheel(pitch, QPoint(0, -12));
    const bool textEntryIntact = !pitch->isReadOnly() && pitchEdit->text() == pendingText &&
                            pendingText.contains(pitch->locale().toString(4.5)) &&
                            pitchValue() == pitchBefore;
    if (!textEntryIntact) {
        std::fprintf(stderr, "Inspector entry: readonly=%d text='%s' pending='%s' value=%g before=%g focus=%s\n",
                     int(pitch->isReadOnly()), qPrintable(pitchEdit->text()),
                     qPrintable(pendingText), pitchValue(), pitchBefore,
                     QApplication::focusWidget() ? QApplication::focusWidget()->metaObject()->className() : "none");
    }
    if (!inspectorCheck(textEntryIntact,
                        "focused text entry survives refresh and both wheel types")) return false;
    pitchKey(Qt::Key_Return);
    if (!inspectorCheck(pitchValue() == 4.5 && pitch->isReadOnly(),
                        "Enter submits typed value and returns to drag mode")) return false;
    m_controller.undo();
    m_inspector->syncFromModel();
    if (!inspectorCheck(pitchValue() == pitchBefore,
                        "one undo reverses typed entry")) return false;
    pitchMouse(QEvent::MouseButtonDblClick, 0);
    pitchKey(Qt::Key_7, QStringLiteral("7"));
    pitchKey(Qt::Key_Escape);
    if (!inspectorCheck(pitchValue() == pitchBefore && pitch->isReadOnly() &&
                            pitch->value() == pitchBefore,
                        "Escape discards unsubmitted text")) return false;
    pitchKey(Qt::Key_Return);
    if (!inspectorCheck(!pitch->isReadOnly(), "keyboard can start text entry")) return false;
    pitchKey(Qt::Key_Escape);

    pitchMouse(QEvent::MouseButtonDblClick, 0);
    pitchKey(Qt::Key_2, QStringLiteral("2"));
    m_inspector->setSelection(m_selectedTrackId, {});
    if (!inspectorCheck(pitchValue() == 2.0 && pitch->isReadOnly(),
                        "selection change finishes entry against the old clip")) return false;
    m_controller.undo();
    m_inspector->setSelection(m_selectedTrackId, QString::fromStdString(audioClip));
    moreClip->setChecked(false);
    clipScroll->verticalScrollBar()->setValue(0);
    QApplication::processEvents();
    ...
    m_timeline->ensureLaneVisible(rowOf(audioTrack));
    { QEventLoop settle; QTimer::singleShot(60, &settle, &QEventLoop::quit); settle.exec(); }
    const auto waitForStaticPaint = [this](std::uint64_t before) {
        QElapsedTimer wait; wait.start();
        while (m_timeline->staticFramePaintCountForTest() <= before && wait.elapsed() < 300) {
    ...
    waitForStaticPaint(waveformPaintsBefore);
    ...
    send(QEvent::MouseMove, bentFadeHandle, Qt::NoButton,
         Qt::LeftButton, Qt::NoModifier);
    waitForStaticPaint(paintsBeforeFadeDrag);
    audioAfter = m_controller.audioClip(audioTrack, audioClip);
    ...
    // The timeline leaves the right button to context menus; dragging it over
    // clips must not borrow the eraser used by the Piano Roll.
    ...
    send(QEvent::MouseButtonRelease, eraseSecond, Qt::RightButton,
         Qt::NoButton, Qt::NoModifier);
    QApplication::processEvents();
    const bool firstKept = hasClip(tracks[1], first);
    const bool secondKept = hasClip(tracks[2], second);
    if (!firstKept || !secondKept) {
        std::fprintf(stderr,
                     "right-button timeline drag erased clips (first=%d second=%d)\n",
                     int(firstKept), int(secondKept));
        return false;
    }
...
bool MainWindow::checkSettingsViewportForTest() {
    ...
    // rendering the physical event lands in QQuickWindow first; in
    ...
    if (tabs && tabs->count() > SettingsWindow::kThemesTab) {
        ...
        if (themeScroll && target &&
            themeScroll->verticalScrollBar()->maximum() > 0) {
            themeScroll->ensureWidgetVisible(target, 0, 40);
            QApplication::processEvents();
            QScrollBar* bar = themeScroll->verticalScrollBar();
            const int beforeWheel = bar->value();
            const int direction = beforeWheel < bar->maximum() ? -1 : 1;
            const int sliderValue = target->value();
            const QPointF targetPoint(target->rect().center());
            const QPointF global = target->mapToGlobal(targetPoint);
            auto* surface = centralWidget()
                ? centralWidget()->findChild<ui::graphics::WorkspaceSurface*>()
                : nullptr;
            const auto sendWheel = [&](QPoint pixels, QPoint angles) {
                QObject* receiver = target;
                QPointF local = targetPoint;
                if (surface && surface->quickWindow()) {
                    receiver = surface->quickWindow();
                    local = surface->quickWindow()->mapFromGlobal(global);
                }
                QWheelEvent wheel(local, global, pixels, angles, Qt::NoButton,
                                  Qt::NoModifier, Qt::ScrollUpdate, false);
                QApplication::sendEvent(receiver, &wheel);
            };
            sendWheel({}, QPoint(0, direction * 120));
            const int afterAngle = bar->value();
            sendWheel(QPoint(0, direction * 24), {});
            const int afterPixels = bar->value();
            const auto movedInDirection = [direction](int from, int to) {
                return direction < 0 ? to > from : to < from;
            };
            liveWheelRouting = movedInDirection(beforeWheel, afterAngle) &&
                movedInDirection(afterAngle, afterPixels) &&
                target->value() == sliderValue;
            if (!liveWheelRouting) {
                std::fprintf(stderr,
                    "settings live wheel failed (gpu %d, max %d, angle %d, "
                    "pixel %d, slider %d/%d, target %s)\n",
                    int(surface != nullptr),
                    bar->maximum(), afterAngle, afterPixels, target->value(),
                    sliderValue, target->objectName().toUtf8().constData());
            }
        } else {
            std::fprintf(stderr,
                "settings live wheel setup failed (scroll %d, target %d, max %d)\n",
    ...
    auto* gpuToggle = m_settingsWindow->findChild<QCheckBox*>(
        QStringLiteral("GpuWorkspaceEnabled"));
    auto* gpuRestart = m_settingsWindow->findChild<QPushButton*>(
        QStringLiteral("GpuRestartButton"));
    const bool runningGpuMode = ui::graphics::gpuWorkspaceEnabled();
    bool gpuRestartSynced = gpuToggle && gpuRestart &&
        (gpuRestart->isHidden() ==
         (gpuToggle->isChecked() == runningGpuMode));
    if (gpuToggle && gpuRestart) {
        const bool original = gpuToggle->isChecked();
        gpuToggle->setChecked(!runningGpuMode);
        QApplication::processEvents();
        gpuRestartSynced = gpuRestartSynced && !gpuRestart->isHidden();
        gpuToggle->setChecked(runningGpuMode);
        QApplication::processEvents();
        gpuRestartSynced = gpuRestartSynced && gpuRestart->isHidden();
        gpuToggle->setChecked(original);
        QApplication::processEvents();
        gpuRestartSynced = gpuRestartSynced &&
            (gpuRestart->isHidden() == (original == runningGpuMode));
    }
    scrollable = scrollable && gpuRestartSynced;
...
bool MainWindow::checkPositionScrubForTest() {
    ...
    const bool cursorMoved = beforePainted && afterPainted && cursorAfterDrag != cursorBeforeDrag;
...
bool MainWindow::checkAuxiliaryWindowPolicyForTest() {
    ...
    auto* titleBar = m_pianoRollFrame->findChild<QWidget*>("InternalEditorTitleBar");
    const auto dragTitle = [titleBar](const QPoint& delta) {
        const QPoint grab(90, titleBar->height() / 2);
        const QPoint origin = titleBar->mapToGlobal(grab);
        ...
        QMouseEvent release(QEvent::MouseButtonRelease,
                            titleBar->mapFromGlobal(origin + delta), origin + delta,
                            Qt::LeftButton, Qt::NoButton, Qt::NoModifier);
        QApplication::sendEvent(titleBar, &release);
        QApplication::processEvents();
    };
    dragTitle(QPoint(0, -10000));
    const QPoint titlePoint = titleBar->mapTo(m_editorHost, QPoint(90, 18));
    QWidget* titleHit = m_editorHost->childAt(titlePoint);
    const bool aboveHeader = m_editorHost == m_workspace && m_pianoRollFrame->y() == 0 &&
        titleHit && m_pianoRollFrame->isAncestorOf(titleHit) &&
        titlePoint.y() < m_transport->height();
    const QString shotDirectory = qEnvironmentVariable("VLTONE_EDITOR_CHECK_SHOTS");
    if (!shotDirectory.isEmpty()) grab().save(shotDirectory + "/editor-over-header.png");
    dragTitle(QPoint(0, 10000));
    const QRect titleRect(titleBar->mapTo(m_editorHost, QPoint()), titleBar->size());
    const bool parkedSafely = m_editorHost->rect().contains(titleRect) &&
        m_pianoRollFrame->geometry().bottom() > m_editorHost->height() &&
        m_pianoRollFrame->size() == restoredGeometry.size();
    if (!shotDirectory.isEmpty()) grab().save(shotDirectory + "/editor-parked.png");
    dragTitle(restoredGeometry.topLeft() - m_pianoRollFrame->pos());
    const bool broughtBack = m_pianoRollFrame->geometry() == restoredGeometry;
...
bool MainWindow::checkPianoRollForTest() {
    ...
    // their activation events before restoring the editor's shared-strip focus.
    if (auto* surface = findChild<ui::graphics::WorkspaceSurface*>()) {
        auto* inputWindow = surface->quickWindow();
        inputWindow->requestActivate();
        QElapsedTimer activationReady; activationReady.start();
        while (!inputWindow->isActive() && activationReady.elapsed() < 1000) {
    ...
    const bool mergedContext = m_noteContextPanel && m_toolPanel &&
        ...
        m_noteContextPanel->geometry().left() >= 0 &&
        m_noteContextPanel->geometry().right() < m_toolPanel->contextRightEdge() &&
...
bool MainWindow::checkContextSyncForTest() {
    ...
    if (!m_contextPanel->checkAdaptiveLayoutForTest()) return false;
    ...
    // Several real mouse moves in one grab: the model follows every sample,
    // the mixer follows on the next visual frame, and history records endpoints.
    ...
    QElapsedTimer frameWait;
    frameWait.start();
    do {
        QApplication::processEvents(QEventLoop::AllEvents, 10);
        const auto* current = m_controller.project().findTrack(trackId.toStdString());
        if (current && std::abs(m_mixer->faderGainForTest(trackId) - current->volume) < .001) break;
        QThread::msleep(1);
    } while (frameWait.elapsed() < 250);
    ...
    if (!audioClip.isEmpty()) {
        ...
        const bool browserWasLeft = m_browserOnLeft;
        const int browserWidthBefore = m_browserWidth;
        auto* browserHandle = static_cast<ui::ResizeHandle*>(m_browserHandle);
        bool browserBoundsOk = true;
        for (const bool onLeft : {true, false}) {
            setBrowserOnLeft(onLeft, /*persist=*/false);
            browserHandle->onDragStart();
            browserHandle->onDrag(onLeft ? width() / 2 : -width() / 2);
            QApplication::processEvents();
            int from = 0, to = 0;
            browserBoundsOk = browserBoundsOk && contextPanelBounds(from, to) &&
                m_contextPanel->x() >= from &&
                m_contextPanel->x() + m_contextPanel->width() <= to;
            // Restore through the same path; tests use isolated preferences.
            browserHandle->onDrag(onLeft ? browserWidthBefore - m_browserDragStartWidth
                                         : m_browserDragStartWidth - browserWidthBefore);
            QApplication::processEvents();
        }
        setBrowserOnLeft(browserWasLeft, /*persist=*/false);
        QApplication::processEvents();
        if (!browserBoundsOk) {
            std::fprintf(stderr, "browser resize lets the context panel cross a reserved boundary\n");
            return false;
        }
        std::fprintf(stderr, "PASS Context layout: browser resizing on either side preserves bounds\n");
...
void MainWindow::sampleForRecovery() {
    ...
    const bool transportActive = m_controller.isPlaying() || m_controller.isRecording() || m_controller.isCountingIn();
    if (!transportActive && !m_projectFileJob && QApplication::mouseButtons() == Qt::NoButton && !QWidget::mouseGrabber()) {
        ...
        if (m_journalStale || pluginStateChanged) {
            ui::perf::Scope measured("recovery.capture.ms");
            auto snapshot = m_controller.captureIncrementalRecoverySnapshot(
                m_recoveryDirtyTracks, m_recoveryAllTracksDirty || pluginStateChanged || m_recoveryDirtyTracks.empty());
            m_recoveryPluginStateFiles.clear();
            for (const auto& state : snapshot.sharedPluginStates)
                m_recoveryPluginStateFiles.push_back(state->fileName);
            m_journal.requestWrite(std::move(snapshot));
            m_recoveryDirtyTracks.clear(); m_recoveryAllTracksDirty = false;
            m_journalStale = false;
        }
    }
...
QJsonObject MainWindow::telemetrySnapshot(bool detailed) {
    QJsonObject sample = PlatformDiagnostics::processSample();
    const double dsp = m_controller.dspLoad() * 100.0;
    sample.insert(QStringLiteral("dsp_load"), dsp);
    sample.insert(QStringLiteral("dsp_peak"), dsp);
    if (detailed) {
        const auto& project = m_controller.project();
        auto snapshot = telemetry::projectSnapshot(project);
        snapshot.insert(QStringLiteral("audio"), telemetry::audioSnapshot(m_controller));
        snapshot.insert(QStringLiteral("position_seconds"), m_controller.positionSeconds());
        sample.insert(QStringLiteral("snapshot"), snapshot);
        int clips = 0, plugins = 0;
        const auto count = [&plugins](const daw::InsertModel& slot) { if (slot.isLoaded()) ++plugins; };
        for (const auto& slot : project.masterInserts) count(slot);
        for (const auto& track : project.tracks) {
            clips += int(track.clips.size());
            count(track.instrument);
            for (const auto& slot : track.inserts) count(slot);
            for (const auto& slot : track.samplerFx.inserts) count(slot);
            for (const auto& clip : track.clips) {
                for (const auto& slot : clip.inserts) count(slot);
                for (const auto& slot : clip.offlineProcess.chain) count(slot);
            }
        }
        const auto xruns = m_controller.audioXruns();
        sample.insert(QStringLiteral("xruns"), double(xruns[0] + xruns[1] + xruns[2] + xruns[3]));
        ...
        sample.insert(QStringLiteral("plugin_count"), plugins);
        sample.insert(QStringLiteral("playback_state"), m_controller.isPlaying() ? "playing" : "stopped");
        sample.insert(QStringLiteral("recording"), m_controller.isRecording());
        sample.insert(QStringLiteral("foreground"), isActiveWindow());
        sample.insert(QStringLiteral("plugins"), telemetry::pluginAggregates(project));
    }
    return sample;
}
...
void MainWindow::closeEvent(QCloseEvent* event) {
    ...
    if (m_publicationUi && m_publicationUi->active) {
        const auto answer = QMessageBox::question(
            ...
            tr("Cancel the cloud publication and close VLTONE? The saved local "
...
MainWindow::~MainWindow() {
    ...
    delete m_notebookWindow;
    m_notebookWindow = nullptr;
    delete m_webPanel;
    m_webPanel = nullptr;
    delete m_aiPanel;
    m_aiPanel = nullptr;
...
void MainWindow::buildLayout() {
    ...
    m_contextPanel->setHomeAnchorProvider(
        [this](int& centreX) { return contextPanelHomeAnchor(centreX); });
    ...
    connect(m_transport, &TransportBar::readoutGeometryChanged, this,
            &MainWindow::layoutContextPanel);

    // ── Main row: browser | inspector | (arrangement over mixer) ──
    auto* row = new QWidget(workspace);
    m_editorHost = workspace;
    m_editorBody = row;
    ...
    if (!ui::graphics::gpuWorkspaceEnabled() && platform != QLatin1String("offscreen") &&
        platform != QLatin1String("minimal")) {
        workspace->setAttribute(Qt::WA_NativeWindow);
    }
    ...
    m_notebookWidth = std::clamp(QSettings().value("notebook/panelWidth", 440).toInt(), 320, 800);
    m_notebookContainer = new QWidget(central);
    m_notebookContainer->setObjectName(QStringLiteral("NotebookContainer"));
    if (!ui::graphics::gpuWorkspaceEnabled() && platform != QLatin1String("offscreen") && platform != QLatin1String("minimal"))
        m_notebookContainer->setAttribute(Qt::WA_NativeWindow);
    m_notebookContainer->setFixedWidth(m_notebookWidth);
    auto* notebookLayout = new QVBoxLayout(m_notebookContainer);
    notebookLayout->setContentsMargins(0, 0, 0, 0);
    ...
    notebookHandle->onDrag = [this](int delta) {
        ...
        applyRightPanelWidths();
        m_notebookWidth = m_notebookContainer->width();
        QSettings().setValue("notebook/panelWidth", m_notebookWidth);
    };
    m_notebookHandle = notebookHandle;
    m_notebookContainer->hide();
    notebookHandle->hide();
    ...
    // full workspace height. Session disconnects do not hide a cloud project.
    m_sessionStrip = new collab::SessionStatusStrip(this);
    m_sessionStrip->hide();
    connect(m_sessionStrip, &collab::SessionStatusStrip::cloudProjectsRequested,
            this, &MainWindow::onOpenCloudProjects);
    connect(m_sessionStrip, &collab::SessionStatusStrip::invitePeopleRequested,
            this, &MainWindow::onInvitePeople);
    connect(m_sessionStrip, &collab::SessionStatusStrip::joinSessionRequested,
            this, [this] { openJoinSessionDialog(); });
    column->addWidget(m_sessionStrip, 0);
#endif

    shell->addWidget(workspace, 1);
    shell->addWidget(m_notebookContainer);
    shell->addWidget(m_webContainer);
    shell->addWidget(m_aiPanel);

    setCentralWidget(central);
    installGpuSurface(central);
    ...
    connect(m_toolPanel, &ToolPanel::timelineZoomChanged, m_timeline,
            &TimelineWidget::setHorizontalZoom);
    connect(m_timeline, &TimelineWidget::horizontalZoomChanged, m_toolPanel,
            &ToolPanel::setTimelineZoom);

    struct TrackHeightSliderEdit {
        std::vector<std::pair<std::string, double>> before;
        int baseline = ui::kLaneHeight;
        bool active = false;
        bool changed = false;
    };
    const auto trackHeightEdit = std::make_shared<TrackHeightSliderEdit>();
    connect(m_toolPanel, &ToolPanel::trackHeightEditStarted, this,
            [this, trackHeightEdit] {
                if (trackHeightEdit->active) return;
                trackHeightEdit->before.clear();
                trackHeightEdit->before.reserve(
                    m_controller.project().tracks.size());
                for (const auto& track : m_controller.project().tracks)
                    trackHeightEdit->before.emplace_back(track.id, track.height);
                std::vector<int> heights;
                heights.reserve(trackHeightEdit->before.size());
                for (const auto& [id, storedHeight] : trackHeightEdit->before) {
                    (void)id;
                    heights.push_back(ui::laneHeightFor(storedHeight));
                }
                if (!heights.empty()) {
                    const auto middle = heights.begin() + heights.size() / 2;
                    std::nth_element(heights.begin(), middle, heights.end());
                    trackHeightEdit->baseline = *middle;
                }
                trackHeightEdit->active = true;
                trackHeightEdit->changed = false;
            });
    connect(m_toolPanel, &ToolPanel::trackHeightChanged, this,
            [this, trackHeightEdit](int height) {
                if (!trackHeightEdit->active) return;
                const int clamped = std::clamp(height, ui::kMinLaneHeight,
                                               ui::kMaxLaneHeight);
                const double factor = double(clamped) /
                                      std::max(1, trackHeightEdit->baseline);
                bool sampleChanged = false;
                for (const auto& [trackId, storedHeight] :
                     trackHeightEdit->before) {
                    const int next = std::clamp(
                        int(std::lround(ui::laneHeightFor(storedHeight) * factor)),
                        ui::kMinLaneHeight, ui::kMaxLaneHeight);
                    const auto* track =
                        m_controller.project().findTrack(trackId);
                    if (!track || ui::laneHeightFor(track->height) == next)
                        continue;
                    m_controller.setTrackHeight(trackId, double(next));
                    sampleChanged = true;
                }
                if (!sampleChanged) return;
                trackHeightEdit->changed = true;
                // Both columns read the same document heights. Refresh them in
                // this input frame so the lane edge never runs ahead of its
                // header while the slider is moving.
                if (m_trackList) m_trackList->syncRowHeights();
                if (m_timeline) {
    ...
    connect(m_toolPanel, &ToolPanel::trackHeightEditFinished, this,
            [this, trackHeightEdit] {
                if (!trackHeightEdit->active) return;
                bool finalChanged = false;
                if (trackHeightEdit->changed) {
                    for (const auto& [trackId, storedHeight] :
                         trackHeightEdit->before) {
                        const auto* track =
                            m_controller.project().findTrack(trackId);
                        if (track && std::abs(track->height - storedHeight) >
                                         1.0e-9) {
                            finalChanged = true;
                            break;
                        }
                ...
                if (finalChanged) {
                    m_controller.commitTrackHeightEdit(trackHeightEdit->before);
                    markDirty();
                }
                trackHeightEdit->before.clear();
    ...
    connect(m_trackList, &TrackListWidget::trackValuesChanged, this,
        [this](const QStringList& ids, bool dirty, bool appearance) {
            if (dirty) markDirty(ids);
            queueUiChange(ui::UiChangeSet::values(ids, appearance));
        });
    ...
    connect(m_trackList, &TrackListWidget::trackHeightChanged, this, [this] {
        ...
        if (m_toolPanel) {
            std::vector<int> heights;
            heights.reserve(m_controller.project().tracks.size());
            for (const auto& track : m_controller.project().tracks)
                heights.push_back(ui::laneHeightFor(track.height));
            if (!heights.empty()) {
                const auto middle = heights.begin() + heights.size() / 2;
                std::nth_element(heights.begin(), middle, heights.end());
                m_toolPanel->setTrackHeightValue(*middle);
            }
        }
        markDirty();
    });
    ...
    connect(m_inspector, &InspectorWidget::edited, this, [this](bool localFileDirty) {
        if (localFileDirty) markDirty();
        queueUiChange(ui::UiChangeSet::values({m_selectedTrackId}, true));
        for (SampleEditorWindow* editor : m_sampleEditors)
            if (editor && editor->isVisible()) editor->refresh();
    });
    ...
    connect(m_contextPanel, &ContextPanel::liveEdited, this, [this] {
        queueUiChange(ui::UiChangeSet::values(m_trackList->selectedTrackIds(), true));
    });
    ...
    connect(m_transport, &TransportBar::notebookToggled, this,
            [this](bool on) { setNotebookVisible(on); });
...
void MainWindow::syncViews() {
    ...
    if (m_trackList) {
        m_trackList->rebuild();
        if (!m_trackList->selectedTrackIds().contains(m_selectedTrackId))
    ...
    if (m_timeline) {
        ...
        m_timeline->reloadTimedTextSettings();
        m_timeline->update();
    }
    if (m_inspector) {
    ...
    if (m_pianoRoll) m_pianoRoll->refresh();
    if (m_notebookWindow) m_notebookWindow->syncFromProject();
    for (SampleEditorWindow* editor : m_sampleEditors) {
        if (editor) editor->refresh();
    }
...
void MainWindow::refreshAfterCollaborationProjection(
    ...
    if (collaborationChangeMovesTrackRows(impact)) {
        ...
        QStringList ids;
        for (const auto& id : impact.trackIds) ids.push_back(QString::fromStdString(id));
        if (impact.masterGainChanged) ids.push_back({});
        auto change = ui::UiChangeSet::values(ids, impact.timelineChanged);
        change.structure = impact.graphRebuild;
        change.clipContent = collaborationChangeTouchesClipContent(impact);
        change.fullArrangement = impact.timelineChanged && impact.trackIds.empty();
        change.transport = impact.transportProjectionChanged;
        queueUiChange(change);
    }
}
...
void MainWindow::markDirty(const QStringList& tracks) {
    if (tracks.isEmpty()) m_recoveryAllTracksDirty = true;
    else for (const auto& id : tracks) m_recoveryDirtyTracks.insert(id.toStdString());
    m_dirty = true;
    ++m_projectRevision;
...
void MainWindow::ensureNotebook() {
    if (m_notebookWindow || !m_notebookContainer) return;
    m_notebookWindow = new NotebookWindow(&m_controller, m_notebookContainer);
    m_notebookContainer->layout()->addWidget(m_notebookWindow);
    connect(m_notebookWindow, &NotebookWindow::settingsRequested, this,
            [this] { openSettings(SettingsWindow::kNotebookTab); });
    connect(m_notebookWindow, &NotebookWindow::closeRequested, this,
            [this] { setNotebookVisible(false); });
    connect(m_notebookWindow, &NotebookWindow::detachRequested, this,
            [this] { setNotebookDetached(!m_notebookDetached); });
    connect(m_notebookWindow, &NotebookWindow::timedTextChanged, this,
            [this] {
                if (m_timeline) m_timeline->reloadTimedTextSettings();
            });
    connect(m_notebookWindow, &NotebookWindow::projectContentChanged,
            this, [this] { markDirty(); });
    if (m_notebookWindow->importedLegacyContent()) markDirty();
    if (QSettings().value("notebook/detached", false).toBool())
        setNotebookDetached(true);
}

void MainWindow::setNotebookVisible(bool visible, bool persist) {
    if (!m_notebookContainer) return;
    if (visible) ensureNotebook();
    m_notebookContainer->setVisible(visible && !m_notebookDetached);
    m_notebookHandle->setVisible(visible && !m_notebookDetached);
    if (m_notebookDetachedWindow) {
        if (visible && m_notebookDetached) {
            m_notebookWindow->show();
            presentAuxiliaryWindow(m_notebookDetachedWindow);
        } else {
            if (m_notebookDetached)
                QSettings().setValue("notebook/geometry", m_notebookDetachedWindow->saveGeometry());
            m_notebookDetachedWindow->hide();
        }
    }
    if (visible && !m_notebookDetached && m_notebookWindow) m_notebookWindow->show();
...
void MainWindow::setNotebookDetached(bool detached) {
    if (!m_notebookWindow || detached == m_notebookDetached) return;
    m_changingNotebookPlacement = true;
    if (detached) {
        if (!m_notebookDetachedWindow) {
            m_notebookDetachedWindow = new QDialog(this, Qt::Window);
            m_notebookDetachedWindow->setObjectName(QStringLiteral("NotebookDetachedWindow"));
            m_notebookDetachedWindow->setWindowTitle(tr("Notebook"));
            m_notebookDetachedWindow->resize(640, 760);
            m_notebookDetachedWindow->setMinimumSize(340, 540);
            installGpuSurface(m_notebookDetachedWindow);
            auto* layout = new QVBoxLayout(m_notebookDetachedWindow);
            layout->setContentsMargins(0, 0, 0, 0);
            layout->setSpacing(0);
            const QByteArray geometry = QSettings().value("notebook/geometry").toByteArray();
            if (!geometry.isEmpty()) m_notebookDetachedWindow->restoreGeometry(geometry);
            connect(m_notebookDetachedWindow, &QDialog::finished, this, [this] {
                if (!m_changingNotebookPlacement) setNotebookVisible(false);
            });
        }
        m_notebookContainer->layout()->removeWidget(m_notebookWindow);
        m_notebookWindow->setParent(m_notebookDetachedWindow);
        m_notebookDetachedWindow->layout()->addWidget(m_notebookWindow);
    } else {
        QSettings().setValue("notebook/geometry", m_notebookDetachedWindow->saveGeometry());
        m_notebookDetachedWindow->layout()->removeWidget(m_notebookWindow);
        m_notebookWindow->setParent(m_notebookContainer);
        m_notebookContainer->layout()->addWidget(m_notebookWindow);
        m_notebookDetachedWindow->hide();
    }
    m_notebookDetached = detached;
    m_notebookWindow->setDetached(detached);
    QSettings().setValue("notebook/detached", detached);
    m_changingNotebookPlacement = false;
...
void MainWindow::applyRightPanelWidths() {
    ...
    const bool notebookVisible = m_notebookContainer && !m_notebookContainer->isHidden();
    ...
    int notebookEffective = notebookVisible ? std::clamp(m_notebookWidth, 320, 800) : 0;
    int overflow = std::max(0, webEffective + aiEffective + notebookEffective - panelBudget);
    ...
    if (notebookVisible && overflow > 0) {
        const int take = std::min(overflow, notebookEffective - 320);
        notebookEffective -= take;
        overflow -= take;
    }
    ...
    if (notebookVisible && overflow > 0)
        notebookEffective -= std::min(overflow, notebookEffective);
    if (notebookVisible) m_notebookContainer->setFixedWidth(notebookEffective);
...
bool MainWindow::contextPanelAnchor(int& centreX) const {
    ...
    // the plate at its stable home under the transport readout.
    int left = 0, right = 0;
...
bool MainWindow::contextPanelHomeAnchor(int& centreX) const {
    if (!m_transport || !m_toolPanel) return false;
    centreX = m_toolPanel
                  ->mapFromGlobal(m_transport->readoutCenterGlobal())
                  .x();
    return true;
}
...
bool MainWindow::contextPanelBounds(int& left, int& right) const {
    ...
    right = std::min(right, m_toolPanel->contextRightEdge());
    left = std::min(right, std::max(left, m_toolPanel->contextLeftEdge()));
    // An exhausted strip is still a real bound, not permission to fall back
    // to the full window and cover the browser or track controls.
    return true;
}
...
bool MainWindow::checkRemoteCommandMetadataForTest(QString* error) const {
    return m_shortcuts && m_shortcuts->checkRemoteMetadata(error);
}
...
void MainWindow::buildSemanticCommands() {
    ...
    auto bind = [this](const char* rawId, const QString& label,
        ...
        m_shortcuts->registerCommand(id, label, category, shortcut, action,
                                     std::move(metadata));
    ...
    bind("tool.automationCreationToggle", tr("Toggle Automation Creation Mode"),
         kTools, tr("Toggle double-click automation creation for parameters."),
         QStringLiteral("automation.creation-mode"), Risk::Safe,
         [this] {
             m_automationCreationLatched = !m_automationCreationLatched;
             updateAutomationCreationMode();
         }, QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_U));
    const auto updateAutomationShortcut = [this] {
        m_toolPanel->setAutomationCreationShortcut(
            m_shortcuts->shortcut(QStringLiteral("tool.automationCreationToggle"))
                .toString(QKeySequence::NativeText));
    };
    connect(m_shortcuts, &ShortcutManager::changed, this, updateAutomationShortcut);
    updateAutomationShortcut();
...
void MainWindow::buildMenus() {
    ...
    if (m_collaboration) {
        ...
        connect(m_startSessionAction, &QAction::triggered, this,
                &MainWindow::onStartCollaborationSession);
        QAction* joinByCode = session->addAction(tr("Join a Session…"));
        joinByCode->setToolTip(
            tr("Enter an invitation code to join someone else's session"));
        connect(joinByCode, &QAction::triggered, this,
        ...
        connect(m_endSessionAction, &QAction::triggered, this, [this] {
            ...
            const auto answer = QMessageBox::question(
                ...
                tr("End this session for everyone? VLTONE will keep the room "
...
bool MainWindow::probePluginEditorSwap(const QString& firstName,
    ...
    auto settle = [] {
        // processEvents() returns as soon as the queue is empty, so a tight
        // loop can finish before the editor's 80 ms settle timer ever becomes
        // due. Keep a real event loop alive long enough for native mapping,
        // attachment, close/deleteLater and the orphan sweep.
        QEventLoop loop;
...
void MainWindow::placePlayheadForShot(double seconds, bool rolling) {
    ...
    if (rolling) {
        ...
        if (m_timeline) m_timeline->seedPlayheadTrailForShot(30.0);
    }
    syncViews();
}
...
bool MainWindow::checkLayoutIndependentShortcuts() {
    ...
    const QStringList semanticIds = {
        ...
        QStringLiteral("tool.automationCreationToggle"),
...
bool MainWindow::checkAudioAnalysisDialogForTest() {
    return checkAudioAnalysisResultDialogForTest(this);
}
...
bool MainWindow::checkBrowser(const QString& folder, const QString& audioFile,
    ...
    if (!m_browser->previewVisibleForTest())
        return fail("audio preview area stayed hidden");
    ...
    // MIDI selections use the same lower preview area but render their notes,
    // and parsing must arrive through the worker without blocking selection.
    if (!m_browser->showFolderForTest(folder, midiFile, /*persist=*/false))
        return fail("MIDI file was not found in the browser tree");
    decodeTimeout.restart();
    while (!m_browser->hasMidiPreviewForTest() &&
           decodeTimeout.elapsed() < kDecodeTimeoutMs) {
        QApplication::processEvents(QEventLoop::AllEvents, 10);
        QThread::msleep(1);
    }
    if (!m_browser->hasMidiPreviewForTest())
        return fail("MIDI selection did not draw its notes");
    if (!m_browser->previewVisibleForTest())
        return fail("MIDI preview area stayed hidden");
    if (!m_browser->clearFileSelectionForTest())
        return fail("preview area remained after file selection was cleared");
    ...
    // real destination).
    {
        ...
        const bool selectedTemplate = m_browser->showFolderForTest(
            QFileInfo(templatePath).absolutePath(), templatePath, false);
        const bool templateKind = m_browser->selectedProjectTemplateForTest();
        if (!templateResults.contains(templatePath) || exposedInternal || !selectedTemplate || !templateKind) {
            std::fprintf(stderr, "browser template detail: search=%d internal=%d selected=%d kind=%d expected=%s results=%s\n",
                templateResults.contains(templatePath), exposedInternal, selectedTemplate, templateKind,
                templatePath.toUtf8().constData(), templateResults.join(QLatin1Char('|')).toUtf8().constData());
            QDir(templatePath).removeRecursively();
...
bool MainWindow::checkPluginAutoOpenForTest() {
    ...
    QApplication::processEvents();
    if (auto* search = instrumentMenu->findChild<QLineEdit*>(
            QStringLiteral("PluginPickerSearch"))) {
        search->setText(QStringLiteral("Sampler"));
        QApplication::processEvents();
    }
...
bool MainWindow::checkPluginSearchFocusForTest() {
    ...
    QHash<QAction*, QList<QKeySequence>> shortcutsBeforePopup;
    for (const auto& command : m_shortcuts->commands()) {
        if (command.action)
            shortcutsBeforePopup.insert(command.action,
                                        command.action->shortcuts());
    }

    menu->popup(mapToGlobal(QPoint(16, 16)));
    QApplication::processEvents();
    const bool focusedOnOpen = edit->hasFocus();
    bool bareApplicationShortcutsSuppressed = true;
    for (const auto& command : m_shortcuts->commands()) {
        if (!command.action) continue;
        for (const QKeySequence& sequence : command.action->shortcuts()) {
            if (sequence.count() == 1 &&
                sequence[0].keyboardModifiers() == Qt::NoModifier) {
                bareApplicationShortcutsSuppressed = false;
                break;
            }
        }
        if (!bareApplicationShortcutsSuppressed) break;
    }
    ...
    delete menu;
    QApplication::processEvents();
    bool shortcutsRestored = true;
    for (auto it = shortcutsBeforePopup.cbegin();
         it != shortcutsBeforePopup.cend(); ++it) {
        if (it.key()->shortcuts() != it.value()) {
            shortcutsRestored = false;
            break;
        }
    }
    return focusedOnOpen && bareApplicationShortcutsSuppressed && routed &&
           shortcutStayedInSearch && shortcutsRestored &&
...
void MainWindow::openSettings(int tab) {
    if (!m_settingsWindow) {
        ...
        connect(m_settingsWindow, &SettingsWindow::notebookSettingsChanged, this,
                [this] {
                    if (m_notebookWindow) m_notebookWindow->reloadSettings();
                    if (m_timeline) m_timeline->reloadTimedTextSettings();
                });
        connect(m_settingsWindow,
                &SettingsWindow::themeBackgroundSettingsChanged, this,
                [this] {
                    if (m_timeline) m_timeline->reloadBackgroundSettings();
                    if (m_transport) m_transport->reloadBackgroundSettings();
                });
                ...
                [this] {
                    ...
                    if (QProcess::startDetached(
                        ...
                        QMessageBox::warning(
                            ...
                            tr("VLTONE could not start a new process. "
...
void MainWindow::openPianoRoll(const QString& trackId, const QString& clipId) {
    if (!m_pianoRoll) {
        ...
        if (m_noteContextPanel) {
            ...
            m_noteContextPanel->setHomeAnchorProvider(
                [this](int& centreX) {
                    return contextPanelHomeAnchor(centreX);
                });
...
void MainWindow::buildStatusBar() {
    ...
    m_cpuStatusButton = cpuStatus;
    cpuStatus->setObjectName(QStringLiteral("CpuLoadStatus"));
    cpuStatus->setAccessibleName(tr("Audio processing CPU load"));
    cpuStatus->setCheckable(true);
    cpuStatus->setAutoRaise(true);
    cpuStatus->setCursor(Qt::PointingHandCursor);
    cpuStatus->setFocusPolicy(Qt::StrongFocus);
    cpuStatus->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    auto* cpuRow = new QHBoxLayout(cpuStatus);
    cpuRow->setContentsMargins(6, 2, 6, 2);
    cpuRow->setSpacing(5);
    ...
    // The whole block is one mouse/keyboard target in either display mode.
    for (QWidget* child : {static_cast<QWidget*>(m_cpuStatusIcon),
                           static_cast<QWidget*>(m_statusRight),
                           static_cast<QWidget*>(m_cpuStatusMeter)})
        child->setAttribute(Qt::WA_TransparentForMouseEvents);
    cpuRow->addWidget(m_cpuStatusIcon);
    cpuRow->addWidget(m_cpuStatusMeter, 0, Qt::AlignVCenter);
    cpuRow->addWidget(m_statusRight);

    // The project text takes spare room; the CPU control stays at the right
    // edge and never stretches into a second readout in the middle.
...
QProgressBar::chunk { border-radius: 3px; background: %2; }
    ...
    const auto applyCpuMode = [this](bool meter) {
        ...
        m_cpuStatusButton->setToolTip(hint);
        m_cpuStatusButton->setAccessibleDescription(hint);
    };
    const bool meterMode = QSettings().value(
        ui::kCpuStatusMeterModeSetting, false).toBool();
    cpuStatus->setChecked(meterMode);
    applyCpuMode(meterMode);
    connect(cpuStatus, &QToolButton::toggled, this,
            [applyCpuMode](bool meter) {
                applyCpuMode(meter);
                QSettings().setValue(ui::kCpuStatusMeterModeSetting, meter);
            });
...
bool MainWindow::routeEditChord(EditChord chord) {
    if (m_notebookWindow && m_notebookWindow->ownsEditorFocus()) {
        const QString command = chord == EditChord::Cut ? QStringLiteral("cut")
                              ...
                              : chord == EditChord::Paste ? QStringLiteral("paste")
                              : chord == EditChord::Delete ? QStringLiteral("delete")
                              : QString();
        return m_notebookWindow->handleEditorCommand(command);
    }
...
void MainWindow::installGpuSurface(QWidget* source) {
    ui::graphics::GraphicsPreferences::instance().watchCompatibilityWindow(source,
        [this, source](ui::graphics::FrameStats& frame) {
            for (const auto xruns : m_controller.audioXruns()) frame.audioXruns += xruns;
            frame.frameBudgetMs = ui::FrameClock::instance().periodSeconds(source) * 1000.;
        });
    if (!source || !ui::graphics::gpuWorkspaceEnabled() ||
        source->property("vlt.gpuSurfaceActive").toBool()) return;
    auto* surface = new ui::graphics::WorkspaceSurface(source);
    surface->setProjectRevisionProvider([this] { return m_controller.projectRevision(); });
    connect(&ui::FrameClock::instance(), &ui::FrameClock::preferenceChanged,
            surface, &ui::graphics::WorkspaceSurface::refreshPresentationMode);
    connect(&ThemeManager::instance(), &ThemeManager::changed,
            surface, &ui::graphics::WorkspaceSurface::invalidate);
    surface->setFrameContextProvider([this, source](ui::graphics::FrameStats& frame) {
        for (const auto xruns : m_controller.audioXruns()) frame.audioXruns += xruns;
        frame.frameBudgetMs = ui::FrameClock::instance().periodSeconds(source) * 1000.;
    });
    connect(surface, &ui::graphics::WorkspaceSurface::frameMeasured, this,
            [](double preparation, double synchronization, double render, double submission, double renderThreadCpu) {
        ui::perf::sample("gpu.scene.prepare.ms", preparation);
        ui::perf::sample("gpu.scene.sync.ms", synchronization);
        ui::perf::sample("gpu.scene.render.cpu.ms", render);
        if (submission > 0) ui::perf::sample("gpu.scene.submission.interval.ms", submission);
        if (renderThreadCpu >= 0) ui::perf::sample("gpu.scene.render.thread.cpu.ms", renderThreadCpu);
    });
    connect(surface, &ui::graphics::WorkspaceSurface::failed, this,
            [this, source, guard = QPointer<ui::graphics::WorkspaceSurface>(surface)](const QString& reason) {
        // Defer teardown until recording/event delivery has unwound.
        if (guard) guard->deleteLater();
        statusBar()->showMessage(tr("GPU workspace unavailable: %1. Using compatibility rendering.").arg(reason), 10000);
    }, Qt::QueuedConnection);
}
...
InternalEditorFrame* MainWindow::hostInternalWindow(
    ...
    if (ui::graphics::gpuWorkspaceEnabled()) {
        auto* pluginEditor = qobject_cast<PluginEditorWindow*>(content);
        if (pluginEditor && pluginEditor->requiresNativeSurface()) {
            // A vendor NSView/HWND has to live above the Quick scene. Keep
            // exactly this editor frame native; allowing Qt to promote its
            // ancestors turns the complete workspace into native siblings and
            // permanently breaks the Quick container's stacking after close.
            frame->prepareForNativeSurface();
        }
    }
    frame->setWorkspaceArea(m_editorBody);
...
void MainWindow::presentInternalWindow(QWidget* content) {
    ...
    if (!frame) {
        if (content->property("dawAuxiliaryWindow").toBool()) presentAuxiliaryWindow(content);
        return;
    }
    content->show();
    frame->present();
...
bool MainWindow::eventFilter(QObject* watched, QEvent* ev) {
    switch (ev->type()) {
        ...
        case QEvent::KeyRelease: {
            ...
            // Ctrl borrows the arrangement's secondary tool, including while
            // a text field or the web panel owns focus.
...
void MainWindow::updateAutomationCreationMode() {
    ui::setAutomationCreationMode(m_automationCreationLatched);
    if (m_toolPanel) m_toolPanel->setAutomationCreationActive(m_automationCreationLatched);
}
...
void MainWindow::stopCloudRecordingNow(bool interactiveError) {
    ...
    for (const auto& track : m_cloudRecording->finalized.tracks) {
        ...
        capture.status = daw::recovery::classifyCloudRecordingCaptureStatus(
            track.fileWriteSucceeded && !track.interrupted && track.inputXruns == 0, track.capturedFrames,
    ...
    if (mergeConflict) {
        const QString message = tr(
            "The finished take conflicts with existing recovery metadata. VLTONE kept both the audio and track reservations for a safe retry.");
        m_preserveCloudRecoverySession = true;
        statusBar()->showMessage(message, 9000);
        if (interactiveError)
...
void MainWindow::onDuplicateSelectedTrack() {
    ...
    std::string copy;
    const bool withClips = ui::duplicateTrackClips();
    if (track->kind == daw::TrackKind::Pattern) {
        copy = m_controller.duplicatePattern(trackId.toStdString(), withClips);
    } else {
        copy = m_controller.duplicateTrack(trackId.toStdString(),
                                           /*withInserts=*/true, withClips);
    }
    if (copy.empty()) return;

    const QString copyId = QString::fromStdString(copy);
    syncViews();
    // The freshly created root is the new editing context. Updating the list's
    // selection as well as MainWindow's primary id keeps the visual highlight,
    // the inspector and the next rapid Cmd+D on the same track.
    if (m_trackList) m_trackList->setSelectedTrack(copyId);
...
void MainWindow::queueUiChange(const ui::UiChangeSet& change) {
    m_pendingUiChange.merge(change);
    if (!m_uiChangeTimer) {
        m_uiChangeTimer = new ui::FrameTimer(this);
        connect(m_uiChangeTimer, &ui::FrameTimer::timeout, this, [this] {
            m_uiChangeTimer->stop();
            auto changes = std::exchange(m_pendingUiChange, {});
            if (changes.structure) { syncViews(); return; }
            const QStringList ids = changes.tracks.values();
            if (!ids.isEmpty()) {
                if (m_trackList) m_trackList->syncTrackValues(ids);
                if (m_mixer) m_mixer->syncFromModel(ids);
                if (changes.tracks.contains(m_selectedTrackId)) {
                    if (m_inspector) m_inspector->syncFromModel();
                    if (m_contextPanel) m_contextPanel->refresh();
                }
            }
            if (m_timeline) {
                if (changes.fullArrangement) m_timeline->update();
                else for (const auto& id : changes.arrangementTracks) m_timeline->invalidateTrack(id);
            }
            if (changes.clipContent) {
                if (m_pianoRoll) m_pianoRoll->refresh();
                for (auto* editor : m_automationEditors.values()) editor->refresh();
                m_selection.refresh();
                layoutContextPanel();
            }
            if (changes.transport && m_transport) m_transport->refresh();
        });
    }
    m_uiChangeTimer->start();
}
...
void MainWindow::showNextDownloadedAudioPrompt() {
    ...
    if (choice != DownloadedAudioDialog::Choice::None &&
        !ui::prepareAudioImport(this, m_controller, pending.path))
        choice = DownloadedAudioDialog::Choice::None;
...
bool MainWindow::checkWebBrowserForTest(const QString& audioFile) {
    ...
    const auto waitForDrawer = [&] {
        QElapsedTimer timeout; timeout.start();
        // Wait for the observable final state, not a sleep barely longer than
        // the animation. Busy CI/build hosts can defer the final timer event.
        while (timeout.elapsed() < 1500) {
            QApplication::processEvents(QEventLoop::AllEvents, 5);
            QThread::msleep(1);
            if (!leftDock) break;
            const auto animations = leftDock->findChildren<QAbstractAnimation*>();
            if (std::none_of(animations.cbegin(), animations.cend(),
                            [](QAbstractAnimation* a) { return a->state() == QAbstractAnimation::Running; })) break;
        }
    };
    waitForDrawer();
    ...
    raise();
    activateWindow();
    // Activate the actual input window before assigning QWidget focus. The
    // raster shell may still be inactive after native dialogs and hide/show;
    // setting child focus alone does not activate its embedded Quick window.
    QWindow* inputWindow = windowHandle();
    if (auto* surface = findChild<ui::graphics::WorkspaceSurface*>())
        inputWindow = surface->quickWindow();
    if (inputWindow) inputWindow->requestActivate();
    QElapsedTimer activationReady; activationReady.start();
    while (inputWindow && !inputWindow->isActive() && activationReady.elapsed() < 1000) {
        QApplication::processEvents(QEventLoop::AllEvents, 10);
        QThread::msleep(1);
    }
    address->setFocus(Qt::OtherFocusReason);
    QElapsedTimer focusReady; focusReady.start();
    while (QApplication::focusWidget() != address && focusReady.elapsed() < 1000) {
        QApplication::processEvents(QEventLoop::AllEvents, 10);
        QThread::msleep(1);
    }
    if (QApplication::focusWidget() != address) {
        std::fprintf(stderr, "web self-test: address focus was not established (active=%d visible=%d)\n",
                     inputWindow && inputWindow->isActive(), address->isVisible());
        return false;
    }
...
QString MainWindow::chooseProjectTemplate() {
    ui::ProjectTemplateOpenDialog dialog(ui::projecttemplates::files(), this);
    const int result = dialog.exec();
    if (dialog.libraryChanged() && m_browser) m_browser->reloadSettings();
    return result == QDialog::Accepted ? dialog.selectedPath() : QString();
}
...
bool MainWindow::createProjectFromTemplatePath(const QString& packageDir) {
    return loadProjectTemplatePath(packageDir, /*startup=*/false);
}

bool MainWindow::loadProjectTemplatePath(const QString& packageDir,
                                         bool startup) {
#ifdef DAW_ENABLE_COLLABORATION
    if (!prepareCloudRecordingForProjectTransition()) return false;
#endif
    if (packageDir.isEmpty() || (!startup && !maybeSaveChanges())) return false;
    ...
    if (!result) {
        ...
        if (startup) {
            qWarning().noquote()
                << "Startup template could not be opened:"
                << QString::fromStdString(result.message());
            statusBar()->showMessage(
                tr("Startup template “%1” is unavailable; opened an empty project")
...
bool MainWindow::openConfiguredStartupTemplate() {
    const QString path = ui::startupproject::templatePath();
    if (path.isEmpty()) return false;
    if (!QFileInfo(path).isDir() ||
        !ui::projecttemplates::isTemplatePackage(path)) {
        statusBar()->showMessage(
            tr("Startup template “%1” is unavailable; opened an empty project")
...
bool MainWindow::checkStartupTemplateForTest() {
    ...
    ui::startupproject::setTemplatePath(package);
    const bool loaded = openConfiguredStartupTemplate() &&
        m_controller.project().tracks.size() == 1 &&
        m_controller.project().tracks.front().name == "Startup Vocal";

    // File > New Project remains the existing clean-project command and must
    // never reapply the launch preference that is still selected above.
    onNewProject();
    const bool newProjectStayedBlank =
        m_controller.project().tracks.size() == 1 &&
        m_controller.project().tracks.front().name != "Startup Vocal";
    ...
    return loaded && newProjectStayedBlank;
}
...
bool MainWindow::openExternalPath(const QString& sourcePath) {
    ...
    const bool isProjectDirectory = info.isDir() &&
        QFileInfo(QString::fromStdString(daw::ProjectSerializer::manifestPath(
                      path.toStdString())))
            .isFile();
...
bool MainWindow::openProjectPath(const QString& path) {
    ...
    const QScopedValueRollback<bool> fileJob(m_projectFileJob, true);
    struct PreparedOpen {
        daw::EngineController::PreparedProject project;
        audio::Result result = audio::Result::ok();
    };
    audio::Result r = audio::Result::fail(audio::EngineError::Unknown, "Open interrupted");
    try {
        auto prepared = ui::prepareInBackground<PreparedOpen>(this, tr("Preparing project…"),
            [path = dir.toStdString(), rate = m_controller.sampleRate()](const auto& keepGoing) {
                PreparedOpen prepared;
                prepared.result = daw::EngineController::prepareProjectOpen(path, rate, prepared.project, keepGoing);
                return prepared;
            });
        if (!prepared) return false;
        r = prepared->result;
        if (r) r = m_controller.openPreparedProject(std::move(prepared->project));
    } catch (const std::exception& error) {
...
bool MainWindow::doSave(const QString& packageDir) {
    ...
    try {
        ...
        r = audio::Result::fail(audio::EngineError::FileWriteError, error.what());
    }
    const bool unchanged = savedUiRevision == m_projectRevision &&
                           savedControllerRevision == m_controller.projectRevision();
    if (r && unchanged) m_controller.acceptPreparedProjectSave(prepared->project);
    if (!r) {
...
void MainWindow::onSaveProjectTemplate() {
    ...
    const QString currentArtwork =
        QString::fromStdString(m_controller.project().coverImagePath);
    ui::ProjectTemplateSaveDialog dialog(initial, currentArtwork, this);
    if (dialog.exec() != QDialog::Accepted) return;
    const ui::ProjectTemplateSaveOptions options = dialog.options();
    const QString name = options.name;
    ...
    if (!options.artworkPath.isEmpty()) {
        statusBar()->showMessage(tr("Adding template artwork…"));
        QApplication::processEvents(QEventLoop::ExcludeUserInputEvents);
        QApplication::setOverrideCursor(Qt::WaitCursor);
        QString artworkError;
        const bool artworkInstalled = ui::projecttemplates::installArtwork(
            path, options.artworkPath, &artworkError);
        QApplication::restoreOverrideCursor();
        if (!artworkInstalled) {
            QMessageBox::warning(
                this, tr("Template Artwork Failed"),
                tr("The template was saved, but its artwork could not be added.\n\n%1")
                    .arg(artworkError));
        }
    }
...
bool MainWindow::saveProjectAs() {
    ...
    const QString parentDirectory = m_projectPath.isEmpty()
        ? QString()
    ...
    const QString target = absoluteCleanPath(options.packagePath());
    if (QFileInfo::exists(target) &&
        (m_projectPath.isEmpty() ||
         absoluteCleanPath(m_projectPath) != target)) {
        const auto replace = QMessageBox::question(
            this, tr("Replace Project"),
            tr("A project named “%1” already exists in this location. Replace it?")
                .arg(options.name),
            QMessageBox::Save | QMessageBox::Cancel, QMessageBox::Cancel);
        if (replace != QMessageBox::Save) return false;
    }

    const std::string previousAuthor = project.author;
    const std::string previousCover = project.coverImagePath;
    m_controller.setProjectMetadata(options.author.toStdString(),
                                    options.coverPath.toStdString());
    if (doSave(target)) return true;
    m_controller.setProjectMetadata(previousAuthor, previousCover);
    return false;
}
...
void MainWindow::refreshPlayheadFrame() {
    ...
    if (!moving || (!arrangementVisible && !pianoVisible)) {
        ...
        if (wasActive && !moving) {
            ...
            // The piano roll has a clock on its own top-level window.
        }
        return;
    }
...
void MainWindow::syncPlayheadTimer() {
    ...
    const bool pianoVisible = m_pianoRoll && m_pianoRoll->isVisible() &&
                              !m_pianoRoll->isMinimized();
    if (m_pianoRoll && !m_pianoPlayheadTimer) {
        m_pianoPlayheadTimer = new ui::FrameTimer(m_pianoRoll, this);
        connect(m_pianoPlayheadTimer, &ui::FrameTimer::timeout, this, [this] {
            if (m_pianoRoll) m_pianoRoll->refreshPlayhead();
        });
    }
    if (m_pianoPlayheadTimer) {
        m_pianoPlayheadTimer->setSurface(m_pianoRoll);
        if (moving && pianoVisible) m_pianoPlayheadTimer->start();
        else {
            const bool wasActive = m_pianoPlayheadTimer->isActive();
            m_pianoPlayheadTimer->stop();
            if (wasActive && pianoVisible) m_pianoRoll->refreshPlayhead();
        }
    }
    if (moving && arrangementVisible) {
...
void MainWindow::refreshUi() {
    ...
    if (m_controller.offlineRenderInProgress()) return;
    {
        ui::perf::Scope phase("refreshUi.actions.ms");
    ...
    {
        ui::perf::Scope phase("refreshUi.transport.ms");
        if (m_contextPanel) m_contextPanel->followSelection();
        m_transport->refresh();
    }
    if (m_mixer && m_mixer->isVisible()) m_mixer->syncMeterTimer();
    if (!m_controller.isPlaying() && !m_controller.isRecording()) {
        if (m_trackList && m_trackList->isVisible()) m_trackList->refreshMeters();
        if (m_inspector && m_inspector->isVisible()) m_inspector->refreshMeters();
    }
    const bool automationPlaying = m_controller.isPlaying();
    {
        ui::perf::Scope phase("refreshUi.automation.ms");
    ...
    {
        ui::perf::Scope phase("refreshUi.status.ms");
        ...
        if (m_cpuStatusMeter && m_cpuStatusMeter->value() != std::min(cpuPercent, 100)) {
            // QProgressBar::setValue calls repaint() synchronously. On macOS
            // that flushed the native window inside this tick and blocked the
            // GUI for up to 10 ms while Quick was submitting its frame. Keep
            // the value/accessibility update immediate, and coalesce painting
            // with the next normal QWidget update instead.
            const bool updates = m_cpuStatusMeter->updatesEnabled();
            if (updates) m_cpuStatusMeter->setUpdatesEnabled(false);
            m_cpuStatusMeter->setValue(std::min(cpuPercent, 100));
            if (updates) m_cpuStatusMeter->setUpdatesEnabled(true);
        }
        const QString right = QStringLiteral("%1%").arg(cpuPercent);
        if (m_statusRight->text() != right) {
            m_statusRight->setText(right);
            const QString description = tr("Audio CPU load: %1 percent").arg(cpuPercent);
            m_statusRight->setAccessibleName(description);
            if (m_cpuStatusButton) m_cpuStatusButton->setAccessibleName(description);
        }
    }
