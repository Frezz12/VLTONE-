...
int main(int argc,char** argv){
 if(argc!=2)return 2;
 // Read-only discovery; the test never opens an input stream. Restore the
 ...
 for(auto d:devices){CFStringRef uid=nullptr;bytes=sizeof(uid);p.mSelector=kAudioDevicePropertyDeviceUID;
  if(!AudioObjectGetPropertyData(d,&p,0,nullptr,&bytes,&uid)&&uid){if(CFStringCompare(uid,CFSTR("BuiltInSpeakerDevice"),0)==kCFCompareEqualTo)device=d;CFRelease(uid);}}
 if(!device)return 3;
 ...
 struct Restore{AudioDeviceID device;UInt32 frames;~Restore(){AudioObjectPropertyAddress p{kAudioDevicePropertyBufferFrameSize,kAudioObjectPropertyScopeGlobal,kAudioObjectPropertyElementMain};
  auto status=AudioObjectSetPropertyData(device,&p,0,nullptr,sizeof(frames),&frames);std::printf("RESTORE hardware_frames=%u status=%d\n",frames,int(status));}} restore{device,original};
 daw::EngineController c;audio::AudioDeviceConfig config;
 config.sampleRate=48000;config.bufferSize=512;config.inputEnabled=false;config.outputDeviceUid="coreaudio:BuiltInSpeakerDevice";
 ...
 int failed=0;
 for(unsigned frames:{32u,16u,128u}){
  ...
  if(!okay)failed=8;
  std::printf("HARDWARE frames=%u hardware_frames=%u workers=%u realtime_helpers=%u workgroup_helpers=%u callbacks=%llu p95_ms=%.4f p99_ms=%.4f max_ms=%.4f over=%llu xruns=%llu,%llu,%llu,%llu input_enabled=%d\n",frames,actual,c.audioWorkerCount(),c.realtimeAudioWorkerCount(),c.workgroupAudioWorkerCount(),(unsigned long long)(d.diagCallbackCount()-count),s.p95Ms,s.p99Ms,s.maximumMs,(unsigned long long)(c.callbackMetrics().counters().overruns-overruns),(unsigned long long)(y[0]-x[0]),(unsigned long long)(y[1]-x[1]),(unsigned long long)(y[2]-x[2]),(unsigned long long)(y[3]-x[3]),c.audioConfiguration().inputEnabled);std::fflush(stdout);
 }
