...
export default async function ReleasesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="releases-main">
    <header className="releases-hero"><span className="release-eyebrow"><PackageOpen size={15} aria-hidden />VLTONE</span><h1>{ru ? "Обновления" : "Releases"}</h1><p>{ru ? "Новые возможности, изменения и исправления — вместе с доступными установщиками для каждой системы." : "New features, changes, fixes, and the installers currently available for each platform."}</p></header>
