...
const chartText = { fill: "#9da1b8", fontSize: 11 };
const tooltipStyle = { background: "rgba(9, 11, 24, .96)", border: "1px solid rgba(255, 255, 255, .14)", borderRadius: 14 };
  ...
  return <AdminShell>
    ...
    {data && <>
      <div className="vlt-grid vlt-grid-2 dashboard-charts">
        <section className="vlt-card vlt-card-pad vlt-stack" aria-label="График запусков и крашей за 24 часа"><h2 className="vlt-section-title">Запуски и краши · 24 часа</h2><div className="chart-frame"><ResponsiveContainer width="100%" height="100%"><AreaChart data={activity}><defs><linearGradient id="activityFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#8b5cf6" stopOpacity={0.48} /><stop offset="100%" stopColor="#8b5cf6" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid stroke="rgba(255,255,255,.07)" vertical={false} /><XAxis dataKey="label" tick={chartText} minTickGap={24} /><YAxis tick={chartText} allowDecimals={false} width={30} /><Tooltip contentStyle={tooltipStyle} /><Area type="monotone" dataKey="sessions" name="Запуски" stroke="#a78bfa" strokeWidth={2} fill="url(#activityFill)" /><Area type="monotone" dataKey="crashes" name="Краши" stroke="#fb7185" strokeWidth={2} fill="transparent" /></AreaChart></ResponsiveContainer></div></section>
        <section className="vlt-card vlt-card-pad vlt-stack" aria-label="График расхода AI токенов за месяц"><h2 className="vlt-section-title">AI-расход · текущий UTC-месяц</h2><div className="chart-frame"><ResponsiveContainer width="100%" height="100%"><BarChart data={ai}><CartesianGrid stroke="rgba(255,255,255,.07)" vertical={false} /><XAxis dataKey="label" tick={chartText} minTickGap={20} /><YAxis tick={chartText} width={52} tickFormatter={(value) => new Intl.NumberFormat("ru", { notation: "compact" }).format(Number(value))} /><Tooltip contentStyle={tooltipStyle} formatter={(value) => new Intl.NumberFormat("ru").format(Number(value))} /><Bar dataKey="tokens" name="Токены" fill="#22d3ee" radius={[8, 8, 3, 3]} /></BarChart></ResponsiveContainer></div></section>
      </div>
