...
test("administrator searches a user and performs protected account actions", async ({ page }) => {
  ...
  const user = { id, email: "tester@example.com", nickname: "Тестировщик", locale: "ru", status: "active", consent_version: "2026-08-23", created_at: "2026-08-23T00:00:00Z" };
  ...
  await page.route("**/api/v1/admin/**", async (route) => {
    ...
    if (path.endsWith(`/users/${id}`) && request.method() === "GET") return route.fulfill({ json: { user, devices: [], quota, subscription: { plan: { display_name: "Demo" } }, counts: { launches: 1, crashes: 0, bugs: 0 } } });
    ...
    if (path.endsWith("/admin/users")) { search = url.searchParams.get("q") ?? search; return route.fulfill({ json: { users: [user] } }); }
    return route.fulfill({ status: 204 });
  });
